...
#ifndef SC_MOUNT_SUPPORT_FAKE_OPTION
	/* 
	 * this part code is allowed to run only in case of mount utility support `-f' 
	 * (fake) option, which "Causes everything to be done except for the actual 
	 * system call" (see `man mount`). if `-f` is not support, mount will fail which
	 * cause the whole processing game over. 
	 */
