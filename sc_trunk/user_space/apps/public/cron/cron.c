...
#ifdef LOGFILE
	/* Open our log file */
	if ((fd = open(LOGFILE, O_CREAT | O_WRONLY | O_APPEND | O_TRUNC, 0600)) == -1)
		return;
	write(fd, buf, l);
	close(fd);
#endif
...
static inline int decode_env(char *p) {
	...
	*q++ = '\0';
	...
	if (*q == '\0') {		/* Clear variable */
		...
		/*stoenv seems not work? no time for check*/
		/* 2004 10 05 Ron fix memory leek */
		if(strcmp(p,"TZ")==0){
			unsetenv("TZ");
			//setenv("TZ",q,1);
		}else if(strcmp(p,"DAYLIGHT")==0){
			unsetenv("DAYLIGHT");
			//setenv("DAYLIGHT",q,1);
		}
		stoenv(p, q);
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		time(&t);		/* Work out what time it is now */
		now = localtime(&t);
		/* fix for daylight saving time */
		
//		printf("daylight==%s\n",getenv("DAYLIGHT"));
		if(getenv("DAYLIGHT")){
			if(strcmp(getenv("DAYLIGHT"),"1")==0)
			{
		//		printf("time now =%d\n",now->tm_hour);
				now->tm_hour++;
		//		printf("time now =%d\n",now->tm_hour);
