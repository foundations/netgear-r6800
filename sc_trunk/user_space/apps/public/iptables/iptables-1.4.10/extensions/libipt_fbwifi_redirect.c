...
struct reject_names {
	const char *name;
	const char *alias;
	enum ipt_fbwifi_redirect_with with;
	const char *desc;
};

static const struct reject_names reject_table[] = {
	{"fbwifi-redirect", "fbwifi-redirect",
		IPT_FBWIFI_REDIRECT, "FB WiFi redirect page"}
};
...
{
	printf(
"fbwifi_redirect target options:\n"
...
{
	struct ipt_fbwifi_redirect_info *reject = (struct ipt_fbwifi_redirect_info *)t->data;

	/* default */
	reject->with = IPT_FBWIFI_REDIRECT;
...
{
	struct ipt_fbwifi_redirect_info *reject = (struct ipt_fbwifi_redirect_info *)(*target)->data;
...
static void REJECT_print(const void *ip, const struct xt_entry_target *target,
                         int numeric)
{
	const struct ipt_fbwifi_redirect_info *reject
		= (const struct ipt_fbwifi_redirect_info *)target->data;
	unsigned int i;
...
static struct xtables_target reject_tg_reg = {
	.name		= "fbwifi_redirect",
	.version	= XTABLES_VERSION,
	.family		= NFPROTO_IPV4,
	.size		= XT_ALIGN(sizeof(struct ipt_fbwifi_redirect_info)),
	.userspacesize	= XT_ALIGN(sizeof(struct ipt_fbwifi_redirect_info)),
	.help		= REJECT_help,
	.init		= REJECT_init,
	.parse		= REJECT_parse,
	.print		= REJECT_print,
