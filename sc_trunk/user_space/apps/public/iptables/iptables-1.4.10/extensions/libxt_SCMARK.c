              ...
              const void *entry, struct xt_entry_target **target)
{
	...
	case '1':
		if (!xtables_strtoui(optarg, NULL, &mark, 0, UINT32_MAX))
			xtables_error(PARAMETER_PROBLEM, "Bad SCMARK value \"%s\"", optarg);
		markinfo->mark = mark;
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "SCMARK target: Can't specify --set-mark twice");
		*flags = 1;
		break;
	case '2':
		xtables_error(PARAMETER_PROBLEM,
			   "SCMARK target: kernel too old for --and-mark");
	case '3':
		xtables_error(PARAMETER_PROBLEM,
			   "SCMARK target: kernel too old for --or-mark");
	default:
		return 0;
	}
...
static void MARK_check(unsigned int flags)
{
	if (!flags)
		xtables_error(PARAMETER_PROBLEM,
		           "SCMARK target: Parameter --set/and/or-mark"
			   " is required");
}
                         ...
                         const void *entry, struct xt_entry_target **target)
{
	...
	case 'X': /* --set-xmark */
	case '=': /* --set-mark */
		xtables_param_act(XTF_ONE_ACTION, "SCMARK", *flags & F_MARK);
		xtables_param_act(XTF_NO_INVERT, "SCMARK", "--set-xmark/--set-mark", invert);
		if (!xtables_strtoui(optarg, &end, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "SCMARK", "--set-xmark/--set-mark", optarg);
		if (*end == '/')
	...
	case '&': /* --and-mark */
		xtables_param_act(XTF_ONE_ACTION, "SCMARK", *flags & F_MARK);
		xtables_param_act(XTF_NO_INVERT, "SCMARK", "--and-mark", invert);
		if (!xtables_strtoui(optarg, NULL, &mask, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "SCMARK", "--and-mark", optarg);
		info->mark = 0;
		info->mask = ~mask;
		break;
	...
	case '|': /* --or-mark */
		xtables_param_act(XTF_ONE_ACTION, "SCMARK", *flags & F_MARK);
		xtables_param_act(XTF_NO_INVERT, "SCMARK", "--or-mark", invert);
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "SCMARK", "--or-mark", optarg);
		info->mark = value;
		info->mask = value;
		break;
	...
	case '^': /* --xor-mark */
		xtables_param_act(XTF_ONE_ACTION, "SCMARK", *flags & F_MARK);
		xtables_param_act(XTF_NO_INVERT, "SCMARK", "--xor-mark", invert);
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "SCMARK", "--xor-mark", optarg);
		info->mark = value;
		info->mask = 0;
		break;
...
{
	if (flags == 0)
		xtables_error(PARAMETER_PROBLEM, "SCMARK: One of the --set-xmark, "
                          ...
                          int numeric)
{
	...
	if (info->mark == 0)
		printf("SCMARK and 0x%x ", (unsigned int)(u_int32_t)~info->mask);
	else if (info->mark == info->mask)
		printf("SCMARK or 0x%x ", info->mark);
	else if (info->mask == 0)
		printf("SCMARK xor 0x%x ", info->mark);
	else if (info->mask == 0xffffffffU)
		printf("SCMARK set 0x%x ", info->mark);
	else
		printf("SCMARK xset 0x%x/0x%x ", info->mark, info->mask);
}
