...
{
	struct list_head *index_ptr, *index_ptr2 = NULL, *next;
		...
		struct xtc_handle *handle)
{
	...
	if (!(c = iptcc_find_label(chain, handle))) {
		SC_CFPRINTF("unable to find chain `%s'\n", chain);
	...
	if (!(r = iptcc_alloc_rule(c, e->next_offset))) {
		SC_CFPRINTF("unable to allocate rule for chain `%s'\n", chain);
	...
	if (!iptcc_map_target(handle, r)) {
		SC_CFPRINTF("unable to map target of rule for chain `%s'\n", chain);
...
TC_COMMIT(struct xtc_handle *handle)
{
	...
	if (!newcounters) {
		errno = ENOMEM;
		SC_CFPRINTF("out_free_repl_counters\n");
	...
	if (ret < 0) {
		errno = ret;
		SC_CFPRINTF("out_free_newcounters\n");
