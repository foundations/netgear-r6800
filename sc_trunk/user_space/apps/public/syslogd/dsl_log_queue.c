...
static LIST_HEAD(dsl_lw_head);
static LIST_HEAD(dsl_lw_free);
static struct log_queue_t dsl_log_queue[LOG_QUEUE_NUM];
/* Trace log number in window. */
static int dsl_entires_in_window = 0;

/* Check if ntp got time. */
static inline int dsl_check_time_ok(time_t *now)
{
    struct tm tm_now;
...
int dsl_lq_get_entries_in_window(void)
{
    return dsl_entires_in_window;
}

struct log_queue_t *dsl_lq_get_queue(int qid)
{
    if (qid >= 0 && qid < LOG_QUEUE_NUM)
        return &dsl_log_queue[qid];
    return NULL;
}

static void dsl_lq_free_window(struct log_window_t *pwnd)
{
    lq_debug("entering \n");
    ...
    list_add(&pwnd->list, &dsl_lw_free);
    dsl_entires_in_window--;
}

/* cleanup the log window. */
void dsl_lq_cleanup_window(void)
{
    ...
    list_for_each(entry, &dsl_lw_head) {
...
void dsl_lq_refill_window(struct log_queue_t *q)
{
    ...
    for (i = q->head; i != q->tail; i = q_next(i, q->qlen)) {
        ...
        plog = &q->lq[i];
        for (; curr != &dsl_lw_head; curr = curr->next) {
        ...
        pwnd = dsl_lq_alloc_window(curr != &dsl_lw_head);
        if (!pwnd) {
        ...
        list_add_tail(&pwnd->list, (curr != &pwnd->list) ? curr : &dsl_lw_head);
    }
}

/* `__print_func' is used to format the log message. */
void dsl_lq_print_window(const char *filename, char * (*__print_func)(struct log_entity_t*))
{
    struct log_window_t *pwnd;
    struct log_entity_t *plog;
    struct list_head *entry;
...
int dsl_lq_enqueue(struct log_queue_t *q, const char *msg)
{
    struct log_entity_t *plog;
    struct log_window_t *pwnd;
    struct sysinfo info;
    ...
    time(&now);
    plog->abs_timestamp = (dsl_check_time_ok(&now)) ? now : 0;

    printf("new log@[%d,%d] <%s>, timestamp = %ld \n", q->head, q->tail, plog->msg, plog->timestamp);

    /* add it to log window */
    pwnd = dsl_lq_alloc_window(1);
    pwnd->plog = plog;
    plog->pwnd = pwnd;
    list_add(&pwnd->list, &dsl_lw_head);

    return 0;
}

void dsl_lq_cleanup_queue(struct log_queue_t *q)
{
    q->head = q->tail = 0;
    memset(q->lq, 0, sizeof(q->lq));
}

void dsl_lq_cleanup(void)
{
    int i;

    lq_debug("entering \n");

    dsl_lq_cleanup_window();

    for (i = 0; i < LOG_QUEUE_NUM; i++) {
        dsl_lq_cleanup_queue(&dsl_log_queue[i]);
    }
}

int dsl_lq_init(void)
{
    struct log_window_t *pwnd;
    int i;

    INIT_LIST_HEAD(&dsl_lw_head);
    INIT_LIST_HEAD(&dsl_lw_free);
    ...
    for (i = 0; i < LOG_WINDOW_SIZE; i++) {
        list_add(&pwnd[i].list, &dsl_lw_free);
    }

    for (i = 0; i < LOG_QUEUE_NUM; i++) {
        dsl_log_queue[i].qlen = LOG_QUEUE_SIZE;
    }
