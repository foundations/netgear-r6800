...
static void add_ra_header(struct safe_buffer * sb, struct ra_header_info const * ra_header_info, int cease_adv)
{
	...
	/* if ip6_default_route_flag exist (mean wan received ra with lifetime 0), 
	 * send zero router lifetime 
	 * RFC6204 L-3 ~ L-6 and RFC4191*/
	if (access("/tmp/ip6_default_route_flag", F_OK) == 0) {
		radvert.nd_ra_router_lifetime = 0;
	}
