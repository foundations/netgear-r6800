...
/*
 * <$Id: ne_io.c,v 3.66 2008/01/22 14:05:09 feico Exp $>
 */
...
static int net_ifc_cmp (struct ifconf *ifc1, struct ifconf *ifc2) {
	...
	cp1 = (u_char *)ifc1->ifc_buf;
	cp2 = (u_char *)ifc2->ifc_buf;
...
#ifdef IP6_6RD_CE
/*
 * TOTD open source: do not open new socket for wan iface
 * So add the function which can set different socket attr
 *
 */
Nia *nia_create (void)
{
	...
	/* Need use random port as wan socket */
	srandom (time (NULL));
	wan_port = random ();
	wan_port = (wan_port > 1025 ? wan_port : (wan_port + 1025));
	sin6.sin6_port = htons( wan_port );
	...
	/* Bind UDP socket */
    if(net_mesg_socket (ni, 0/* ipv6/ipv4 both*/) < 0)
        return NULL;
    /* It is only for UDP, needn't open new tcp sock */
    ni->tcp_sock = -1;
...
int net_bind_socketlist (void) {
	...
	for (gl = NI_head->next; gl->list_data; gl = gl->next) {
		...
		if (!ni->sa_p) 
			continue;
#ifdef IP6_6RD_CE
		if (net_mesg_socket (ni, T.ipv6_only) >= 0)
#else
        if (net_mesg_socket (ni) >= 0)
#endif
			s++;
#ifdef IP6_6RD_CE
		if (net_stream_socket (ni, T.ipv6_only) >= 0)
#else
...
#ifdef IP6_6RD_CE
int net_mesg_socket (Nia *ni, int ipv6_only)
#else
...
#ifdef IP6_6RD_CE
    if (ipv6_only && ni->sa_p->sa_family == AF_INET6) {
		const int on = 1;
		/* only receive IPv6 addr from this socket */
		SC_CFPRINTF("only receive IPv6 addr from this socket\n");
...
#ifdef IP6_6RD_CE
int net_stream_socket (Nia *ni, int ipv6_only)
#else
