...
void fwd_mark (struct sockaddr *sa, int up) {
	...
	if (((Fwd *)(T.current_fwd->list_data))->went_down_at) {
		...
		if (!fwd_tmp || !fwd_tmp->sa) {
			...
			syslog (LOG_NOTICE, "No valid forwarder, reset all forwarder in list");
...
G_List *fwd_socketlist ( uint16_t qtype) {
...
/* Sercomm Start */
/* Follow Netgear IPv6 spec. 17.2.3. DNS Proxy  
 * 1. For AAAA or A6 request,
 * 	- Try IPv6 DNS server.
 * 	- If no response or error, then try IPv4 DNS server
 * 
 * 2. For A request,
 * 	- Try IPv4 DNS server.
 * 	- If no response or error, then try IPv6 DNS server
 * */
		...
		if ( (qtype == RT_AAAA || qtype == RT_A6 ) && fwd->sa->sa_family == AF_INET6 ) {
			struct sockaddr *sa;
		...
		}else if ( (qtype == RT_A ) && fwd->sa->sa_family == AF_INET ) {
			struct sockaddr *sa;
