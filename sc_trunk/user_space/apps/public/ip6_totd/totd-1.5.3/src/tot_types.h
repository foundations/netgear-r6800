...
/*
 * A Dnshi-jack Nameserver
 */

typedef struct dnshj {
	char hostname[MAX_DNAME+1];
	char dns_hostname[MAX_DNAME+1]; /* convert to dns format */
   	struct sockaddr *sa;	/* dnshj peer address */
    int sa_len;

	int flag;				/* reserved ipv4 or ipv6 */
} Dnshj;
...
typedef struct context {
	...
	uint16_t timeout;
#ifdef IP6_6RD_CE
    u_char sock_opened; /* mark opened sock or copy */
#endif
