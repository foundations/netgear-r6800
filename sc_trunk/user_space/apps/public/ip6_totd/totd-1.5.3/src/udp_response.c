...
/*
 * <$Id: udp_response.c,v 3.58 2006/01/30 10:57:11 dillema Exp $>
 */
...
int udp_response_start (u_char *mesg_buf, int mesg_len, struct sockaddr *sa_p,
	...
	if (cont->mesg.hdr->opcode == OP_QUERY) {
		...
		case 1:
			...
			return (udp_response_finish (cont, 0));
		case 2:
			/* resolved by dns-hijack */
			SC_CFPRINTF("totd dns hijack \n");
			cont->mesg.hdr->rcode = RC_OK;
			create_response_material(cont);
			return (udp_response_finish (cont, 0));
		case 3:
			/* resolved by dns-hijack */
			SC_CFPRINTF("totd dns hijack - no resp \n");
			cont->mesg.hdr->rcode = RC_OK;
			create_response_material(cont);
			return (udp_response_finish (cont, 1));
		default:
...
int udp_response_finish (Context *cont, int not_send) {
	...
	if (not_send)
	{
		SC_CFPRINTF("not send\n");
		len = -1;
	} else
	{
		/* send the answer: using default open lan fiace sock */
