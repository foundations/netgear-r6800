...
/*
 * <$Id: request.c,v 3.100 2008/01/22 14:05:09 feico Exp $>
 */
...
#define QNAME (cont->mesg.p + sizeof (Mesg_Hdr))
	...
	/* gordon add for dns-hijack */
	if ((cont->sa = dnshj_select(cont->qname)) != NULL)
	{
#ifdef HTTPD_IPV6_SUPPORT
	    /* Now we need support AAAA type */
        if (cont->q_type == 0x1c)   // DNS AAAA
        {           
            cont->sa_len = sizeof(struct in6_addr);
        }
        else  // DNS A
        {
            /* When IPv4 DNS receive, reponse ttl and data is 0*/
            cont->sa_len = 0;
            
		SC_CFPRINTF("silence drop dns request for A request %s, seem response A with null data would cause broswer stop send dns\n", cont->qname);
		return 3;
        }  
#else
		cont->sa_len = sizeof(struct in_addr);
		{
			struct sockaddr *sa_tmp = cont->sa;
			//SC_CFPRINTF("%x\n", *(unsigned int*)cont->sa);
			if (*((unsigned int*)cont->sa) == 0xffffffff)
			{
				SC_CFPRINTF("silence drop dns request for %s, since the mapping ip is all ff\n", cont->qname);
				return 3;
			} else
			{
				SC_CFPRINTF("dns request for %s is dnshj, will response with ip directly but not forward to wan server\n", cont->qname);	
			}
		}
#endif		    
		return 2;
...
#undef QNAME
	 ...
	 */
	cont->nameservers = fwd_socketlist( qtype);
...
int do_forward(struct context *parent, u_char *qname, uint16_t qclass,
	...
	strlcpy ((char *)cont->qname, (const char *)qname, MAX_DNAME);
	cont->q_class = qclass;
	cont->q_type = qtype;
	cont->q_id = mesg_id ();
	...
	if (tcp == 0) {
#ifdef IP6_6RD_CE
        /* Open socket for WAN DNS server: forward Lan DNS request */
        cont->netifaddr = nia_create ();
        cont->sock_opened = 1;
#endif
...
int recursive_process (Context *cont) {
	...
	qclass = cont->q_class;
	qtype = cont->q_type;
	strlcpy((char *)qname, (const char *)cont->qname, MAX_DNAME);
