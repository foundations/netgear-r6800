...
/*
 * <$Id: ne_mesg.c,v 3.50 2008/01/22 14:05:09 feico Exp $>
 */
...
int mesg_insert(G_List *an_list, uint16_t q_type, uint16_t q_class,
		   u_char *owner, uint16_t owner_len, void *sa, int sa_len) 
{
		RRset_Couple *rc;
		RR_List *rrl;
		RRset *rrs;
		uint32_t r_ttl = 0x00000e10;
//		uint16_t rdlen_ex = 4;
//		unsigned char rp_ex[] = {0xc0, 0xa8, 0x00, 0x01};
//		seep("before mesg_insert an_list->next = %p\n", an_list->next);
...
//		rrl = rr_list_add (((RRset_Couple *) (rc))->rrl,
//				   r_ttl, rdlen_ex, rp_ex);
		...
		if (an_list) {
			if (list_add (an_list, rrset_copy (rrs)) < 0) {
				rrset_free (rrs);
				rrset_free (rrs);
				rrset_couple_free(rc);
				goto error;
...
//		seep("end mesg_insert an_list->next = %p\n", an_list->next);
		rrset_free (rrs);
		rrset_couple_free(rc);
error:
	return 0;
