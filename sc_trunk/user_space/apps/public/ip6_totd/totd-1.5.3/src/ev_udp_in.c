...
#ifdef IP6_6RD_CE
/*
 * Now we add DNS request(wan iface) socket, so also need
 * update max_fd and fd_read flag for select function can
 * detect the wan iface socket state is changed
 */
void ev_udp_maxfd(fd_set *fds, int *max_fd)
{
	G_List *gl;

    if( UDPL_head )
    {
    	for (gl = UDPL_head->next; gl->list_data; gl = gl->next)
    	{
    	    Context *cont = NULL;
            Nia *ni = NULL;

            /* Point to Ni part */
    		cont = ((Ev_UDP_In_Data *) gl->list_data)->cptr;
    		ni = cont->netifaddr;

    		if(ni && (ni->udp_sock >= 0) && (cont->sock_opened == 1))
    		{
                /* Config FD flag and */
...
/*
 * Now we add DNS request(wan iface) socket, so also need
 * check receive packets from every new open wan iface
 * socket
 */
void ev_udp_in_check_read(fd_set *fds)
{
	G_List *gl;

    if( UDPL_head )
    {
    	for (gl = UDPL_head->next; gl->list_data; )
    	{
    	    Context *cont = NULL;
            Nia *ni = NULL;

            /* Point to Ni part, now socket is opened by cont
             * parent
             */
    		cont = ((Ev_UDP_In_Data *) gl->list_data)->cptr;
    		ni = cont->netifaddr;
            ...
            /* Config FD flag and */
            if(ni && (ni->udp_sock >= 0) && (cont->sock_opened == 1)
                && FD_ISSET (ni->udp_sock, fds))
    		{
    		    /* clear the FD set flag */
    		    FD_CLR (ni->udp_sock, fds);
    		    if(ev_udp_in_read(ni->udp_sock)<0)
    		    {
    		        syslog (LOG_INFO, "udp service error");
    		        /* If receive packets fail, go on receive next socket */
    		        gl = gl->next;
    		    }
    		    else
    		    {
    		        /* If process OK, the current cont will be
    		         * release, so need reinit point again, maybe
    		         * we may reread, but it is not a problem I
    		         * think
    		         */
    		        gl = UDPL_head->next;
    		    }
