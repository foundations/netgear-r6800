...
/*
 * <$Id: totd.c,v 3.75 2007/02/21 18:57:41 dillema Exp $>
 */
...
int main (int argc, char **argv) {
	...
	/* list of dnshj configure */
	T.dnshj_list = list_init ();
...
void totd_eventloop (void) {
	...
	while (1) {		/* main loop */
		...
		max_fd = MAXNUM (i, max_fd);
#ifdef IP6_6RD_CE
        /* Now also need do action for wan iface socket:
         * receive DNS response from wan iface
         */
        ev_udp_maxfd(&fd_read, &max_fd);
#endif
		if (T.debug > 3)
		...
		if (fdnum < 0) {
			...
			int sock;
#ifdef IP6_6RD_CE
            /* First, check and process all new open wan ifac
             * socket first, then read and process lan iface
             */
            ev_udp_in_check_read(&fd_read);
#endif
            /* Check lan side socket */
...
int totd_exit (int status) {
	...
	ev_signal_finish ();
		...
		if (T.dnshj_list)
				list_destroy (T.dnshj_list, dnshj_freev);
	if (T.pidfile)
		unlink (T.pidfile);
