...
#ifdef IP6_6RD_CE
    /* Now we need use wan open socket to send DNS request to replace lan socekt */
	if (net_mesg_send (cont->netifaddr, cont->mesg.p, cont->mesg_len, sa) < 0) {
#else
...
#ifdef IP6_6RD_CE
    /* Now we need use wan open socket to send DNS request to replace lan socekt */
	len = net_mesg_send (cont->netifaddr, cont->mesg.p, cont->mesg_len, sa);
#else
