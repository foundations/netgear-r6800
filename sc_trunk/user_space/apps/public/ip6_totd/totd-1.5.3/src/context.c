...
void *context_destroy (Context *cont) {
	...
	{
#ifdef IP6_6RD_CE
        /* Now we will open new socket for DNS request(wan iface)
         * so after finish or exit the session, totd need close
         * the socket.
         */
        if(cont->sock_opened == 1)
		    nia_free (cont->netifaddr, 1);
		else /* free memory only */
