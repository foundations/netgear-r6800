...
/*
 * <$Id: read_config.c,v 3.39 2008/01/22 14:05:09 feico Exp $>
 */
...
int read_config (char *config_file) {
	...
	while (fgets (buf, sizeof (buf), config_fp)) {
		...
		} else if (!strcasecmp (args[0], "dnshj")) {
//			seep("dnshj <%s> <%s>\n", args[1], args[2]);
			if (argcnt < 3 || argcnt > 3 ) {
				syslog (LOG_ERR, "line %d: invalid format: dnshj <domain> [IPaddr]", linenum);
				errcnt++;
			} else {
				
				if (dnshj_add (args[1], args[2]) == -1)
					return -1; /* serious trouble */
			}			
		}
