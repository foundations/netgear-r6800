...
relayGotDiscoveryPacket(PPPoEInterface const *iface)
{
    ...
    /*fprintf(stderr,"source-->%02x:%02x:%02x:%02x:%02x:%02x\n",packet.ethHdr.h_source[0],packet.ethHdr.h_source[1],packet.ethHdr.h_source[2],packet.ethHdr.h_source[3],packet.ethHdr.h_source[4],packet.ethHdr.h_source[5]);
    printf("destination-->%02x:%02x:%02x:%02x:%02x:%02x\n",packet.ethHdr.h_dest[0],packet.ethHdr.h_dest[1],packet.ethHdr.h_dest[2],packet.ethHdr.h_dest[3],packet.ethHdr.h_dest[4],packet.ethHdr.h_dest[5]);*/
	...
	break;
    default:
	/*fprintf(stderr,"Discovery packet with unkown code\n");*/
...
relayGotSessionPacket(PPPoEInterface const *iface)
{
    ...
    /*fprintf(stderr,"receive-->%02x:%02x:%02x:%02x:%02x:%02x\n",packet.ethHdr.h_source[0],packet.ethHdr.h_source[1],packet.ethHdr.h_source[2],packet.ethHdr.h_source[3],packet.ethHdr.h_source[4],packet.ethHdr.h_source[5]);*/
    /* Ignore unknown code/version */
    if (packet.ver != 1 || packet.type != 1) {
	return;
    }
	...
	/*fprintf(stderr,"Session packet with code\n");*/
	return;
    }
    ...
    /*printf("-->%02x:%02x:%02x:%02x:%02x:%02x === %02x:%02x:%02x:%02x:%02x:%02x\n",packet.ethHdr.h_source[0],packet.ethHdr.h_source[1],packet.ethHdr.h_source[2],packet.ethHdr.h_source[3],packet.ethHdr.h_source[4],packet.ethHdr.h_source[5],iface->mac[0],iface->mac[1],iface->mac[2],iface->mac[3],iface->mac[4],iface->mac[5]);*/

    /* Validate length */
    if (ntohs(packet.length) + HDR_SIZE > size) {
	/*fprintf(stderr,"Bogus PPPoE length field\n");*/
	dsyslog(LOG_ERR, "Bogus PPPoE length field (%u)",
	       (unsigned int) ntohs(packet.length));
	return;
    }
