...
parsePacket(PPPoEPacket *packet, ParseFunc *func, void *extra)
{
    ...
    if (packet->ver != 1) {
	dsyslog(LOG_ERR, "Invalid PPPoE version (%d)", (int) packet->ver);
	return -1;
    }
    if (packet->type != 1) {
	dsyslog(LOG_ERR, "Invalid PPPoE type (%d)", (int) packet->type);
	return -1;
    }
	     ...
	     void *extra)
{
	...
	dsyslog(LOG_ERR, "PADT: Service-Name-Error: %.*s", (int) len, data);
	fprintf(stderr, "PADT: Service-Name-Error: %.*s\n", (int) len, data);
	break;
    case TAG_AC_SYSTEM_ERROR:
	dsyslog(LOG_ERR, "PADT: System-Error: %.*s", (int) len, data);
	fprintf(stderr, "PADT: System-Error: %.*s\n", (int) len, data);
	break;
    case TAG_GENERIC_ERROR:
	dsyslog(LOG_ERR, "PADT: Generic-Error: %.*s", (int) len, data);
