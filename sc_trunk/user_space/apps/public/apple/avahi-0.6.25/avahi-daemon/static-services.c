...
#else
                /*FIXME: glob faile to read /etc/avahi/services/*.service, 
                  I don't know why, so load files directly*/
                load_file("/etc/avahi/services/afpd.service");
                load_file("/etc/avahi/services/smbd.service");
                load_file("/etc/avahi/services/adisk.service");
#endif
	        break;
        }
