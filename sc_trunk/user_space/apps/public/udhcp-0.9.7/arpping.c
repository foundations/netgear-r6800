...
long diff(struct timespec start, struct timespec end)
{
    ...
    return temp.tv_sec * 1000000L + temp.tv_nsec/1000;
}
...
int arpping(u_int32_t yiaddr, unsigned char *yimac, u_int32_t ip, unsigned char *mac, char *interface)
{
	...
	long	timeout = 500000L; /* 0.5 second */
	int 	optval = 1;
	int	s;			/* socket */
	...
	fd_set		fdset;
	struct timeval	tm;
	struct timespec nowTime, prevTime;
	...
	*((u_int *) arp.tInaddr) = yiaddr;		/* target IP address */
	...
	while (timeout > 0) {
		...
		timeout -= diff(prevTime, nowTime);
		memcpy(&prevTime, &nowTime, sizeof(prevTime));
	}
	close(s);
