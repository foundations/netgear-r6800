...
static int read_opt(char *line, void *arg)
{
	...
	for (i = 0; options[i].code; i++)
		if (!strcmp(options[i].name, opt))
		{
			...
			option = &(options[i]);
			fp2=fopen("/tmp/option","a");
	       if(fp2){
		fprintf(fp2,"%s----------options[i].name\n",options[i].name);
		fclose(fp2);
	}
		}
...
static struct config_kw_arr k_arr[MAX_INTERFACES] = {
	...
	{"interface",	read_str, &(server_config[0].interface),	LAN_LOG_IFNAME},
	...
	{"group_id",	read_u32, &(server_config[0].group_id),	"1"},
	...
	{"interface",	read_str, &(server_config[1].interface),	LAN_LOG_IFNAME},
	...
	{"group_id",	read_u32, &(server_config[1].group_id),	"1"},
		...
		if (leases[i].yiaddr != 0) {
			if (server_config[ifid].remaining) {
				if (lease_expired(&(leases[i]))) {
				    ...
				    lease_time = leases[i].expires - curr;
				}
			}
			else {
			...
			fwrite(leases[i].hostname, MAXHOSTNAMELEN, 1, fp);
			fwrite(leases[i].cid, MAX_CLIENT_LEN, 1, fp);
...
#ifdef RONSCODE
		if (!(add_lease(lease.chaddr, lease.yiaddr, lease.expires, ifid,(char *)lease.hostname,(char *)lease.cid))) {
