...
int main (int argc, char *argv[]) {
    ...
    printf("%-21s%-20s%-18s%-16s%-20s\n", "cid", "Hostname", "Mac Address", "IP-Address", mode == REMAINING ? "Expires in" : "Expires at");
	/*     "00:00:00:00:00:00 255.255.255.255 Wed Jun 30 21:49:08 1993" */
	while (fread(&lease, sizeof(lease), 1, fp)) {
		for (i = 0; i < 7; i++) {
			printf("%02x", lease.cid[i]);
			if (i != 6) printf(":");
		}
		...
		} else printf("%s", ctime((long *)&expires));
	}
	fclose(fp);
