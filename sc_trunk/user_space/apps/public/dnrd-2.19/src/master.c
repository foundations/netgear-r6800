...
static dnsrec_t *name_lookup(char *name)
{
	...
	//if (do_hack_dns && (rec->type == DNS_NAMEIP) &&
	    (strcmp(rec->object.string, hack_dns_name) == 0) && (strcmp(name,NTGR_APPS_URL)!=0) && (strcmp(name,NTGR_APPS_URL_TEMP)!=0)
) {
	    return (rec);
	}else
#endif
 ...
 * any data.
 * -1 mean that caller need silence drop this request
 * 
...
int master_lookup(unsigned char *msg, int len)
{
	 ...
	 * by the master.
	 */
		if (query.type == DNS_TYPE_AAAA) 
		{
			log_debug(2, "query.type:%d match AAAA type\n", query.type);
			if ((rec = name_lookup(query.name)) != NULL) 
			{
				/* regarding AAAA request with ipv4 address
				 * 	- if server not response, PC will try AAAA several times and then use A to query
				 *	- if server response and its no such name, PC will stop query!
				 *	- if server response and its server error or len=0, PC will try A immediately
				 * 
				 * orignal code, before wan up, since device forward AAAA type routerlogin anyway, 
				 * so if wan not up, it will response server error, which is ok;
				 * but if wan up, it will forward and response no such name, which will cause PC stop query A. 
				 * 
				 * So we have to take it correctly:
				 * 1) make server not reponse AAAA routerlogin, but this will make PC wait long time before it can access device
				 * 2) response len=0, which is r7000 way too. 
				 * 
				 * we now use way 2.
				 */
				dnsheader_t *x;
				...
				compile_int(x, 0);
#else
				/* will implement once gui is acessable by ipv6 */
				start_rdata(x);
				compile_long(x, rec->u.nameip.ipnum);
				end_rdata(x);
				compile_dnsrecords(x, query.name);
				...
				SC_CFPRINTF("resp AAAA routerlogin, x->nscount:%d\n", x->nscount);
				return (x->len);
			}
		}
