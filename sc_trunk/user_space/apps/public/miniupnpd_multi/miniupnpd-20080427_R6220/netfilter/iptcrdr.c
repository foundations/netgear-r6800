...
static struct rdr_desc * rdr_desc_list = 0;
    ...
    static void
add_redirect_desc(unsigned short eport, int proto, const char * desc, unsigned int leaseduration)
{
    struct rdr_desc * p;
    size_t l;
    if(desc)
    {
        ...
        if(p)
        {
            ...
            p->leasetime = leaseduration;
            if(p->leasetime != 0)
                p->leasetime+=get_uptime();
            rdr_desc_list = p;
        }
    }
}
        ...
        char * desc, int desclen)
{
    ...
    strncpy(desc, "Unknown", desclen);
}

struct rdr_desc *get_eport_and_protocol_by_desc(char * desc,	unsigned short *eport, short *proto)
{
    struct rdr_desc * p;
    if(!desc)
        return NULL;
    ...
    int
get_redirect_rule_lease(unsigned short eport, int proto, unsigned int *leasetime, unsigned int *leaseduration)
{
    struct rdr_desc *p;
    for(p = rdr_desc_list; p; p = p->next)
    {
...
/* add_redirect_rule2() */
    int
add_redirect_rule2(const char *r_host,const char * ifname, unsigned short eport,
        const char * iaddr, unsigned short iport, int proto,
        const char * desc, unsigned int leaseduration)
{
    int r = addnatrule(r_host,proto, eport, iaddr, iport);
    if(r >= 0)
        add_redirect_desc(eport, proto, desc, leaseduration);
    return r;
}
 ...
 * */
    int
addnatrule(const char *r_host ,int proto, unsigned short eport,
        const char * iaddr, unsigned short iport)
{
    ...
    if(r_host){
        e->ip.src.s_addr=htonl(inet_addr(r_host));
