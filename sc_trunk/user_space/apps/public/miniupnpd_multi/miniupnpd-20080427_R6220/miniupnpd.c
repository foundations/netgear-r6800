...
/* Handler for the SIGUSR2 signal indicating WAN mode change change. */
static void sigusr2(int sig)
{
	char *wan_ifname = GetWanIfName();
	char *wan_mode = GetWanConnMode();
    char *UpnpNotifyInterval = GetUpnpNotifyInterval();

	SYSLOG(LOG_INFO, "received signal %d, WAN mode change", sig);
	ext_if_name = wan_ifname;
    ext_upnp_notifyinterval = UpnpNotifyInterval;

    v.notify_interval = atoi(UpnpNotifyInterval) * 60;
    gNotifyInterval = (v.notify_interval<<1) ;

	modify_rootDesc(wan_mode);
	modify_ssdp_service(wan_mode);
}
...
init(int argc, char * * argv, struct runtime_vars * v)
{
    ...
    // added by Han fixed upnp rule dup in GUI
#ifdef ENABLE_LEASEFILE
    unlink(lease_file);
#endif
    ...
    for(i=1; i<argc; i++)
    {
        ...
        else switch(argv[i][1])
        {
            ...
            case 'u':
                if(i+1 < argc){
                    int len=strlen(uuidvalue+5);
                    strncpy(uuidvalue+5, argv[++i], len + 1);
                    strncpy(uuidvalue1+5, argv[i], len + 1);
                    strncpy(uuidvalue2+5, argv[i], len + 1);
                    strncpy(uuidvalue3+5, argv[i], len + 1);

                    uuidvalue1[len+4] = uuidvalue[len+4]+1;
                    if(uuidvalue1[len+4] == ':')    /* Next value of '9' is ':'. Need to change it to 'a' */
                        uuidvalue1[len+4]='a';
                    if(uuidvalue1[len+4] > 'f')
                        uuidvalue1[len+4]='0';

                    uuidvalue2[len+4] = uuidvalue1[len+4]+1;
                    if(uuidvalue2[len+4] == ':')    /* Next value of '9' is ':'. Need to change it to 'a' */
                        uuidvalue2[len+4]='a';
                    if(uuidvalue2[len+4] > 'f')
                        uuidvalue2[len+4]='0';

                    uuidvalue3[len+4] = uuidvalue2[len+4]+1;
                    if(uuidvalue3[len+4] == ':')    /* Next value of '9' is ':'. Need to change it to 'a' */
                        uuidvalue3[len+4]='a';
                    if(uuidvalue3[len+4] > 'f')
                        uuidvalue3[len+4]='0';
                }else
                    fprintf(stderr, "Option -%c takes one argument.\n", argv[i][1]);
                break;
...
main(int argc, char * * argv)
{
    ...
    long timeofday,lasttimeofday=0,checktime=0,next_expire_rule_time=0;
    int max_fd = -1;
#ifdef USE_MINIUPNPDCTL
    int sctl = -1;
    ...
    while(!quitting)
    {
        ...
        {
            SYSLOG(LOG_DEBUG, "timeofday <%ld> lasttimeofday <%ld> notify_interval <%d>", timeofday, lasttimeofday, v.notify_interval);
            /* the comparaison is not very precise but who cares ? */
            if(!lasttimeofday || timeofday >= (lasttimeofday + v.notify_interval))
            {
                /*
                 * If First time or notify interval comes, send notify.
                 */
                SYSLOG(LOG_DEBUG, "Time is OK! runtime_flags is <0x%08x>", runtime_flags);
                if (GETFLAG(ENABLEUPNPMASK)) {
                    SYSLOG(LOG_DEBUG, "Calling SendSSDPNotifies2");
            ...
            {
                timeout.tv_sec = lasttimeofday + v.notify_interval
                    - timeofday;
            }
        }
        /* remove unused rules */
        next_expire_rule_time = remove_expired_rules();
        if(next_expire_rule_time != 0 && next_expire_rule_time < timeout.tv_sec)
        {
        	timeout.tv_sec = next_expire_rule_time;
        }

        if( v.clean_ruleset_interval
                && (timeofday >= checktime + v.clean_ruleset_interval))
        {
            if(rule_list)
            {
        ...
        while( nextnatpmptoclean_timestamp && (timeofday >= nextnatpmptoclean_timestamp + startup_time))
        {
        ...
        if(nextnatpmptoclean_timestamp && timeout.tv_sec >= (nextnatpmptoclean_timestamp + startup_time - timeofday))
        {
            /*SYSLOG(LOG_DEBUG, "setting timeout to %d sec", nextnatpmptoclean_timestamp + startup_time - timeofday.tv_sec);*/
            timeout.tv_sec = nextnatpmptoclean_timestamp + startup_time - timeofday;
            timeout.tv_usec = 0;
        }
#endif
...
/*need another signal to trigger notify send out, that is wrong.
    Do create notify in signal handler, before upnpevents_selectfds*/
