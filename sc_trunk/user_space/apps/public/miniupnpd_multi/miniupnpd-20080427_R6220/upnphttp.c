...
#ifdef ENABLE_PNPX
/* Send the binary file whith mimetype */
    static void
sendBinary(struct upnphttp * h, char *filename, char *mimetype);
#endif
...
ProcessHttpQuery_upnphttp(struct upnphttp * h)
{
    ...
    else if(strcmp("GET", HttpCommand) == 0)
    {
        ...
        else if(strcmp(WANETHCFG_PATH, HttpUrl) == 0)
        {
            sendXMLdesc(h, genWANELCfg);
        }
        else if(strcmp(WANIPC_PATH, HttpUrl) == 0)
        {
            sendXMLdesc(h, genWANIPCn);
        }
        else if(strcmp(WANPPPC_PATH, HttpUrl) == 0)
        {
            sendXMLdesc(h, genWANPPPCn);
        }
...
#endif
#ifdef ENABLE_PNPX
        else if(strcmp(PNPX_ICON_PATH, HttpUrl)==0)
        {
			sendBinary(h, "/tmp/pnpx.png","image/png");
        }
#endif /* ENABLE_PNPX */
        ...
        else if(strcmp(LANHOSTCFG_PATH, HttpUrl) == 0)
        {
            sendXMLdesc(h, genLANHostCfg);
        }
#endif
        else
        {
...
sendBinary(struct upnphttp * h, char *filename, char *mimetype)
{
    	...
    	if(r < 0)
    	{
    		SYSLOG(LOG_DEBUG, "stat return error");
    		return;
    	}
    	if(!h->res_buf)
    	{
    		templen = sizeof(httpresphead) + 128 + sb.st_size;
    		h->res_buf = malloc(templen);
    		h->res_buf_alloclen = templen;
    		h->res_buflen = snprintf(h->res_buf, h->res_buf_alloclen,
    	            httpresphead, h->HttpVer,
    	            200, "OK",
    	...
    	if(fp)
    	{
    		h->res_buflen += fread(h->res_buf + h->res_buflen,
    							sb.st_size,	sizeof(char), fp);
    		fclose(fp);
    	}
    	else
    	{
