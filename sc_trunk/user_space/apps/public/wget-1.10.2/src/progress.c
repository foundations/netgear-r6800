...
extern char current_url[], current_hex_url[];
...
static int num=0;
extern int down_torrent;
extern char status_file[],ram_status_file[];
      ...
      static const char *short_units[] = { "B/s", "KB/s", "MB/s", "GB/s" };
		...
		if(!down_torrent){
			if(num==20){
				...
				num=0;
      			sprintf (speed_str, " %7.2f%s", dlspeed, short_units[units]);

				DOWN_SetStr( "task_status", "4", ram_status_file);
				DOWN_SetStr( "down_speed", speed_str, ram_status_file);
				DOWN_SetStr( "finish_size", number_to_static_string (size), ram_status_file);
	#if 0
				if(time(NULL)-old_time>=60){
					DOWN_SetStr( "task_status", "4", status_file);
					DOWN_SetStr( "down_speed", speed_str, status_file);
					DOWN_SetStr( "finish_size", number_to_static_string (size), status_file);
					old_time=time(NULL);
				}
	#endif
			}
		...
		if(!down_torrent){
			if(num==20){
				...
				DOWN_SetStr( "task_status", "4", ram_status_file);
				DOWN_SetStr( "down_speed", "", ram_status_file);
				DOWN_SetStr( "finish_size", number_to_static_string (size), ram_status_file);

				if(time(NULL)-old_time>=60){
					DOWN_SetStr( "task_status", "4", status_file);
					DOWN_SetStr( "down_speed", "", status_file);
					DOWN_SetStr( "finish_size", number_to_static_string (size), status_file);
					old_time=time(NULL);
				}
			}
