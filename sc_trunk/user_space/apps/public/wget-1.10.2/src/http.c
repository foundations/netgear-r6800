...
extern char current_url[], error_status[],local_file_name[],torrent_status[],ram_status_file[];
#ifdef __USE_UNICODE__
	extern char code_page[];
#endif
extern char tmpFileName[];
extern int down_ok, down_torrent;
extern uid_t owner_uid;
...
parse_content_disposition (const char *hdr, char **filename)
{
  ...
  while (extract_param (&hdr, &name, &value, ';'))
	...
	/* Make the file name begin at the last slash or backslash. */
        const char *last_slash = (const char *)memrchr (value.b, '/', value.e - value.b);
        const char *last_bs = (const char *)memrchr (value.b, '\\', value.e - value.b);
  ...
  if (!hs->local_file)
    {
     ...
     /* Honor Content-Disposition whether possible. */
      if (!opt.content_disposition
		...
		if(down_torrent)
			WriteTorrentStatus(hs->local_file);
		else{
#ifdef __USE_UNICODE__
			char converted_text[1024] = {0};
			int ret=0;

			ret=do_convert("UTF-8", code_page, hs->local_file, strlen(hs->local_file), converted_text, 1024);
			if(ret)
				strcpy(converted_text, hs->local_file);
			DOWN_SetStr("file_name", converted_text, ram_status_file);
			printf("hs->local_file: %s\n",converted_text);
#else
			DOWN_SetStr("file_name", hs->local_file, ram_status_file);
			printf("hs->local_file: %s\n",hs->local_file);
#endif
		}
#endif
...
\n    The file is already fully retrieved; nothing to do.\n\n"));
      ...
      *dt |= RETROKF;
      ...
      CLOSE_INVALIDATE (sock);	/* would be CLOSE_FINISH, but there
		...
		if(!down_torrent)
			DOWN_SetStr( "task_status", "2", ram_status_file);
		down_ok=1;
      return RETRUNNEEDED;
    }
	  ...
	  if (contlen != -1)
	    {
	      logputs (LOG_VERBOSE, with_thousand_seps (contlen + contrange));
			if(!down_torrent)
				DOWN_SetStr("total_size", number_to_static_string ((contlen + contrange)),ram_status_file);
			else if(contlen + contrange>1024*1024){//Too large torrent file.
				xfree_null (type);
				CLOSE_INVALIDATE (sock);
				return RANGEERR;
      		}
  ...
  /* Open the local file.  */
	if (!output_stream)   {
		...
		if(!ret){
			free(hs->local_file);
			hs->local_file=NULL;
			hs->local_file=malloc(strlen(converted_text)+1);
			if(hs->local_file==NULL){
				CLOSE_INVALIDATE (sock);
				return FOPEN_EXCL_ERR;
			}
			strcpy(hs->local_file, converted_text);
		}
#endif
...
#ifdef __USE_UNICODE__
			if(!ret)
				fp = fopen_excl (converted_text, 1);
			else
#endif
      ...
      if (!fp){
		...
		if(old_errno==EACCES)
			strcpy(error_status, "No right to store the file on the NAS.");
		else if(old_errno==ENOSPC)
			strcpy(error_status, "No free space on the NAS.");
		  CLOSE_INVALIDATE (sock);
		  return FOPENERR;
		}
		chmod(hs->local_file, 0664);
		if(strlen(hs->local_file)<256)
			strcpy(local_file_name, hs->local_file);
    }
  else
    fp = output_stream;
  ...
  *newloc = NULL;
  ...
  if (local_file && *local_file)
  ...
  else
    {
  	...
  	sprintf(tmpFileName, "%s.http", hstat.local_file);
  	opt.output_document = tmpFileName;
  ...
  if (opt.always_rest && file_exists_p (hstat.local_file))
  {
  	logprintf (LOG_VERBOSE, _("Removing exist file %s.\n"), hstat.local_file);
  	unlink(hstat.local_file);
  }
...
#ifdef __USE_UNICODE__
		ret=do_convert("UTF-8", code_page, locf, strlen(locf), converted_text, 1024);
		if(!ret)
			locf=converted_text;
//		printf("locf: %s\n",locf);
#endif
	...
	*dt &= ~SEND_NOCACHE;
	 ...
	 if(!file_exist){
		if (!opt.output_document)
			locf = hstat.local_file;
		if(hstat.local_file && (!locf || strcmp(locf, hstat.local_file))){
			locf = hstat.local_file;
		}
	}
	if(locf){
		...
		if(down_torrent)
			WriteTorrentStatus(hstat.local_file);
		else
			DOWN_SetStr("file_name", hstat.local_file, ram_status_file);
	}
      /* Time?  */
      tms = time_str (NULL);
	...
	*newloc = xstrdup (hstat.newloc);
	...
	case FWRITEERR: case FOPENERR:
	  ...
	  logprintf (LOG_NOTQUIET, _("Cannot write to `%s' (%s).\n"),
		...
		if(errno==EACCES)
			strcpy(error_status, "No right to store the file.");
		else if(errno==ENOSPC)
			strcpy(error_status, "No free space.");
		else if(errno==EDQUOT)
			strcpy(error_status, "Disk quota exceeds.");
	  free_hstat (&hstat);
	  xfree_null (dummy);
	  return err;
	case CONSSLERR:
      ...
      if (!(*dt & RETROKF))
	{
	  ...
	  //sprintf(error_status,"%d %s.",hstat.statcode, escnonprint (hstat.error));
	  if(strlen(hstat.error)<120)
	  	sprintf(error_status,"%d %s.",hstat.statcode, hstat.error);
	  if(hstat.statcode==404)
	  ...
	  if (*dt & RETROKF)
	    {
	      ...
	      logprintf (LOG_NONVERBOSE,
			...
			if(!down_torrent){
				...
				ret=stat(locf, &f_stat);
				if(!ret){
					DOWN_SetStr("total_size", number_to_static_string (f_stat.st_size),ram_status_file);
					printf("Total Size: %s\n", number_to_static_string (f_stat.st_size));
				}
				if(strcmp(tmrate, "0.00 B/s"))
					DOWN_SetStr("down_speed", tmrate, ram_status_file);
		    	    DOWN_SetStr( "task_status", "2", ram_status_file);
		    	    DOWN_SetStr( "finish_size", number_to_static_string (hstat.len), ram_status_file);
	    	}
	    	/* Now use tmp file when downloading, so need rename afte finish */
	    	rename(locf, hstat.local_file);
	    	strcpy(tmpFileName, hstat.local_file);
	    	chmod(locf, 0664);
			chown(locf,owner_uid,501);
	    	down_ok=1;
