...
char current_url[1024]={0}, current_hex_url[1024]={0}, ram_status_file[670]={0}, status_file[1024]={0}, conf_file[512]={0}, torrent_status[256]={0}, encoded_url[1024]={0};
char tmpFileName[256]={0};
const char *exec_name;
#define	TORRENT_TMP_FOLDER	"/tmp"
...
size_t do_convert(const char* to_ces, const char* from_ces,
...
int bt_file=0;
uid_t owner_uid=501;
char error_status[128]={0}, local_file_name[256]={0};
int down_torrent=0;
#ifdef __USE_UNICODE__
	char code_page[8]="CP437";
#endif
void ParseSpecialChar(char *src, char *dest, int flag)//flag: 0 - Not handle '\' and ''', 1 - Handle '\', 2 -  Handle ''', 3 - Handle All
{
	int i=0,j=0,len=0;
	...
	while(i<len){
		if(src[i]=='\"'){
			...
			if(i>0 && (((unsigned char )src[i-1]>=0x80 && (unsigned char )src[i-1]<=0x9f) || ((unsigned char )src[i-1]>=0xe0&&(unsigned char )src[i-1]<=0xef) || ((unsigned char )src[i-1] >= 0xF0 && (unsigned char )src[i-1] <= 0xFC)))
				dest[j++]=src[i];
...
void sig_term(int sig)//Pause Job
{
...
void sig_int(int sig)//Terminate Job
{
	printf("Receive Signal %d\n", sig);
	if(stop_process)
		exit(-1);
...
void CleanFile()
{
	...
	if(!down_ok){
		if(!stop_process){
			DOWN_SetStr( "task_status", "3", ram_status_file);
			if(strlen(error_status)){
				DOWN_SetStr("result", error_status, ram_status_file);
			}
		#ifdef _LOG_DOWN_TASK_
			if(!down_torrent)
				seteuid(0);
			LogDownTaskActions(current_url, DOWN_TASK_FAILED);
		#endif
		}
		else if(stop_process==1){
			DOWN_SetStr( "task_status", "1", ram_status_file);
		}
	}
	if(!down_torrent)
		seteuid(0);
...
#ifdef _LOG_DOWN_TASK_
	if(down_ok)
		LogDownTaskActions(current_url, DOWN_TASK_FIN);
#endif

	remove(pid_file);
	chmod(local_file_name, 0664);
	chown(local_file_name,owner_uid,501);
	if(stop_process!=2)
	{
	    FILE *fp=NULL;
	    int pid=-1;
	    fp=fopen(MANAGER_PID,"rt");
    	if(fp){
    		fscanf(fp,"%d", &pid);
    		fclose(fp);
    	}
...
static int LockFile(void)
{
    ...
    return 0;
    fd = open("/tmp/download.lck", O_WRONLY | O_CREAT | O_EXCL, 0666);
    if (fd == -1)
		return -1;
    close(fd);
	return 0;
...
{
	return;
    unlink("/tmp/download.lck");
}

void WriteTorrentStatus(char *pFileName)
{
	FILE *fp=NULL;

	fp=fopen(torrent_status, "wt");
	if(fp){
		fputs(pFileName, fp);
		fclose(fp);
	}
...
int DOWN_SetStr(char *pKey, char *pVal, char *pFileName)
{
	FILE *fh, *fh1;
	char buf[1024]={0};
	char key[64]={0}, TmpFile[64]="/tmp/wget.XXXXXX";
	int ccode=0,find=0;
	int fd,num=0,ret=0;
	...
	if(access(pFileName, F_OK))
		creat(pFileName, 0644);
	fh=fopen(pFileName,"rt");
	if(!fh){
		remove(TmpFile);
		UnLockFile();
		if(!down_torrent)
	...
	fclose(fh1);
	if(find){
	    sprintf(buf,"/bin/cp -f %s %s 2>/dev/null",TmpFile,pFileName);
		ccode=system(buf);
		if(ccode){
			remove(TmpFile);
			UnLockFile();
...
void sig_hup(int sig)
{
	...
	max_speed = atoi(nvram_safe_get("gd_down_speed"));
	opt.limit_rate=max_speed*1024;
...
main (int argc, char *const *argv)
{
	...
	FILE *fp=NULL;
    //sh_info info;
	char dest_folder[512] = {0};
	char dest_prefix[512] = {0};
	char username[512] = {0};
	char password[512] = {0};
...
Can't timestamp and not clobber old files at the same time.\n"));
  ...
  if (!nurl && !opt.input_filename)
	...
	HexStr2String(argv[argc-1], current_url);
	strcpy(current_hex_url, argv[argc-1]);
	if(!down_torrent){
		sprintf(pid_file, "%s/task.%d",PID_DIR,getpid());
		fp=fopen(pid_file,"wt");
		if(fp){
			fputs(current_url,fp);
			fclose(fp);
		}
		else
			printf("Fail to write PID file %s\n",pid_file);

		//u_pwd=getpwnam(TaskInfo.owner);
		//if(u_pwd && (u_pwd->pw_uid)>=0)
		//	owner_uid=u_pwd->pw_uid;

      	if(strlen(current_hex_url)>255){
      		strncpy(status_file_prefix, current_hex_url, 254);
      		status_file_prefix[254]=0;
      		sprintf(status_file,DOWNLOAD_FOLDER"/%s", nvram_bcm_safe_get("sd_share_dir"), status_file_prefix);
			...
			sprintf(conf_file, DOWNLOAD_FOLDER"/%s/%s", nvram_bcm_safe_get("sd_share_dir"), status_file_prefix, current_hex_url+254);
			sprintf(status_file,DOWNLOAD_FOLDER"/%s/%s.status", nvram_bcm_safe_get("sd_share_dir"), status_file_prefix, current_hex_url+254);
			sprintf(ram_status_file,TASK_STATUS_FOLDER"/%s/%s.status", status_file_prefix, current_hex_url+254);
      	}
      	else
      	{
			sprintf(conf_file, DOWNLOAD_FOLDER"/%s", nvram_bcm_safe_get("sd_share_dir"), current_hex_url);
			sprintf(status_file,DOWNLOAD_FOLDER"/%s.status", nvram_bcm_safe_get("sd_share_dir"), current_hex_url);
			sprintf(ram_status_file,TASK_STATUS_FOLDER"/%s.status",current_hex_url);
		}
	}
	else{
      	if(strlen(current_hex_url)>255){
      		strncpy(status_file_prefix, current_hex_url, 254);
      		status_file_prefix[254]=0;
      		sprintf(status_file,TORRENT_TMP_FOLDER"/%s/",status_file_prefix);
      		if(access(status_file, F_OK))
      			mkdir(status_file,0775);
      		chmod(status_file, 0775);
			sprintf(torrent_status, TORRENT_TMP_FOLDER"/%s/%s", status_file_prefix, current_hex_url+254);
      	}
      	else{
			sprintf(torrent_status, TORRENT_TMP_FOLDER"/%s",current_hex_url);
		}
	}
	ParseSpecialChar(current_url,encoded_url,1);
#if 0
  ...
  DEBUGP (("DEBUG output created by Wget %s on %s.\n\n", version_string,
	...
	if(down_torrent){
		sprintf(dest_folder,TORRENT_FOLDER, nvram_bcm_safe_get("sd_share_dir"));
	}
	else{
		sprintf(dest_folder,dest_prefix);
		//CheckPath(FSH_MNT_ROOT,dest_folder);
	}
	if(strlen(username))
	{
		opt.http_user=opt.ftp_user=opt.user=username;
		opt.http_passwd=opt.ftp_passwd=opt.passwd=password;
	}
	...
	opt.read_timeout=20;
	opt.dns_timeout=20;
	opt.connect_timeout=20;
	opt.always_rest=1;
	opt.content_disposition = 1;
	max_speed = atoi(nvram_safe_get("gd_down_speed"));
//	opt.backup_converted=1;
	...
	if(!down_torrent){
		...
		if(fh){
			...
			PRO_GetInt("network","code_page",&codepage,fh);
			sprintf(code_page, "CP%d",codepage);
	#endif
			fclose(fh);
		}
		opt.limit_rate=max_speed*1024;
	...
	if (opt.output_document){
      	...
      	else
		{
			...
			if(!down_torrent)
			ret=do_convert("UTF-8", code_page, opt.output_document, strlen(opt.output_document), converted_text, 1024);
			else
				ret=-1;
			if(!ret)
				output_stream = fopen (converted_text,
	    	...
	    	{
		      	strcpy(error_status, strerror (errno));
	      		exit (1);
	    	}
  ...
  signal (SIGWINCH, progress_handle_sigwinch);
#endif
#endif /* HAVE_SIGNAL */
	DOWN_SetStr( "task_status", "5", status_file);
	...
	if(strlen(current_url)){
		...
		if ((opt.recursive || opt.page_requisites)
	  		&& url_scheme (current_url) != SCHEME_FTP)
			status = retrieve_tree (current_url);
		else
			status = retrieve_url (current_url, &filename, &redirected_URL, NULL, &dt);
		if(!down_torrent){
			seteuid(0);
			setegid(0);
            /* Email alert for green downloader */
			if(down_ok)
			{
                FILE *fp = fopen("/tmp/gd_msg", "w");
                char *p = dest_prefix+strlen("/tmp/mnt/shares/");

                if(fp)
                {
                    memset(dest_folder, 0, sizeof(dest_folder));
                    sprintf(dest_folder,"%c:%s", *p, p+1);
                    convert_names(dest_folder);
                    fprintf(fp, "Your download task <<%s>> is successfully finished.The download file is saved at <<%s>>. - NETGEAR Downloader\n", tmpFileName, dest_folder);
                    fclose(fp);
                    system("killall -SIGVTALRM syslogd&");
                }
            }
		}
