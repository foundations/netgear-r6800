...
#define LIST_FILENAME ".ftp"

extern char ftp_last_respline[];

extern FILE *output_stream;
extern int output_stream_regular;
extern char current_url[], finished_size[],error_status[],local_file_name[],ram_status_file[],torrent_status[];
...
print_length (wgint size, wgint start, int authoritative)
{
  ...
  if(!down_torrent)
  	DOWN_SetStr( "total_size", number_to_static_string (size),ram_status_file);
  if (size >= 1024)
...
{
  int csock, dtsock, local_sock, res=0;
  ...
  if (!(cmd & DO_LOGIN))
	...
	case FTPRERR:
	  ...
	  fd_close (csock);
	  con->csock = -1;
	  strcpy(error_status, "Error in server response.");//Shearer Lu
	  return err;
	case FTPSRVERR:
	  logputs (LOG_VERBOSE, "\n");
	  ...
	  fd_close (csock);
	  con->csock = -1;
	  strcpy(error_status, "Error in server greeting.");//Shearer Lu
	  return err;
	case WRITEFAILED:
	  logputs (LOG_VERBOSE, "\n");
	...
	case FTPLOGREFUSED:
	  ...
	  fd_close (csock);
	  con->csock = -1;
		strcpy(error_status, "The server refuses login.");//Shearer Lu
	...
	case FTPLOGINC:
	  ...
	  fd_close (csock);
	  con->csock = -1;
	  strcpy(error_status, "Login failed.");//Shearer Lu
	  return FTPLOGINC;
	case FTPOK:
	  if (!opt.server_response)
...
Error in server response, closing control connection.\n"));
	    ...
	    case FTPNSFOD:
	      ...
	      fd_close (csock);
	      con->csock = -1;
	      strcpy(error_status, "No such directory");//Shearer Lu
	      return err;
	    case FTPOK:
	      break;
	    default:
...
Error in server response, closing control connection.\n"));
	...
	case FTPNSFOD:
	  ...
	  fd_close (dtsock);
	  fd_close (local_sock);
	  strcpy(error_status, "No such file");//Shearer Lu
...
Error in server response, closing control connection.\n"));
	...
	case FTPNSFOD:
	  ...
	  fd_close (dtsock);
	  fd_close (local_sock);
	  strcpy(error_status, "No such file or directory");//Shearer Lu
	  return err;
	case FTPOK:
	  break;
	default:
  ...
  /* Open the file -- if output_stream is set, use it instead.  */
	if (!output_stream || con->cmd & DO_LIST) {
#ifdef __USE_UNICODE__
		if(!down_torrent)
		ret=do_convert("UTF-8", code_page, con->target, strlen(con->target), converted_text, 1024);
		else
			ret=-1;
#endif
	  ...
	  logprintf (LOG_NOTQUIET, "%s: %s\n", con->target, strerror(errno));
		...
		else if(old_errno==EDQUOT)
			strcpy(error_status, "Disk quota exceeds.");
		else
			strcpy(error_status, strerror(old_errno));
	  fd_close (csock);
	  con->csock = -1;
	  fd_close (dtsock);
	  fd_close (local_sock);
	  return FOPENERR;
		...
		chmod(con->target, 0664);
		if(strlen(con->target)<256)
			strcpy(local_file_name, con->target);
    }
  else
    fp = output_stream;
  ...
  if (*len)
    ...
    if(down_torrent && expected_bytes>=1024*1024){//Too large torrent file.
		fd_close (csock);
		con->csock = -1;
#ifdef __USE_UNICODE__
		if(!ret)
			remove(converted_text);
		else
#endif
			remove(con->target);
		return FTPRETRINT;
	...
	*len = restval;
	rd_size = 0;
	//printf("expected_bytes=%llu\n",expected_bytes);
	//printf("restval=%llu\n",restval);
	if(restval<expected_bytes){
		if(restval)
...
ftp_loop_internal (struct url *u, struct fileinfo *f, ccon *con)
{
  ...
  if (opt.always_rest && file_exists_p (con->target))
  {
  ...
  if (opt.noclobber && file_exists_p (con->target))
    {
  	...
  	sprintf(tmpFileName, "%s.ftp", con->target);
  ...
  if (!opt.output_document || !strcmp(opt.output_document, LIST_FILENAME)){
    locf = con->target;
 }
  else{
      ...
      if (opt.verbose)
	{
	  ...
	  if (count > 1)
		...
		logprintf (LOG_VERBOSE, "--%s--  %s\n  %s => `%s'\n",
		     tms, hurl, tmp, locf);
		printf("********************************locf=%s\n",locf);
		if(strcmp(locf, LIST_FILENAME)){
			if(!down_torrent){
#ifdef __USE_UNICODE__
				char converted_text[1024] = {0};
				int ret=0;

				ret=do_convert("UTF-8", code_page, con->target, strlen(locf), converted_text, 1024);
				if(ret)
					strcpy(converted_text, locf);
				DOWN_SetStr("file_name", converted_text, ram_status_file);
#else
				DOWN_SetStr( "file_name", con->target, ram_status_file);
#endif
			}
			else
				WriteTorrentStatus(con->target);
		}
#ifdef WINDOWS
	  ws_changetitle (hurl);
#endif
