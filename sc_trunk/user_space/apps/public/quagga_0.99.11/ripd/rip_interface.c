...
rip_request_interface_send (struct interface *ifp, u_char version)
{
  ...
  //sercomm add check SC_RIP_2M when send request.
  //if (version == RIPv2 && if_is_multicast (ifp))
  if (version == RIPv2 && if_is_multicast (ifp) && (ri->sc_rip_type == SC_RIP_2M))
    {
...
rip_interface_down (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (IS_RIP_DEBUG_ZEBRA)
    zlog_debug ("interface %s index %d flags %ld metric %d mtu %d is down",
	       ifp->name, ifp->ifindex, (long int)ifp->flags, ifp->metric, ifp->mtu);
...
#ifdef __SC_BUILD__
  if (rip && ifp)
      for (rp = route_top (rip_table(ifp->table)); rp; rp = route_next (rp))
#else
  if (rip)
  ...
  if ((rip_enable_if_lookup(ifc->ifp->name) >= 0) ||
      (rip_enable_network_lookup2(ifc) >= 0))
	      ...
	      rip_redistribute_add (ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
				    &address, ifc->ifp->ifindex, NULL, ifc->ifp->table, 0, 0);
#else
    rip_redistribute_add(ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
...
rip_apply_address_del (struct connected *ifc) {
  ...
  rip_redistribute_delete(ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
                          &address, ifc->ifp->ifindex, ifc->ifp->table);
#else
  rip_redistribute_delete(ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
...
#ifndef __SC_BUILD__
      /*Martin_huang@sdcsercomm.com:if we can receive multicast packet, we still accept it,  it should be fine*/
      return 0;
#endif
  }
...
#ifdef __SC_BUILD__
	  rip_redistribute_add (ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
				&address, connected->ifp->ifindex, NULL, ifp->table, 0, 0);
#else
          rip_redistribute_add (ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
...
#ifdef __SC_BUILD__
            rip_redistribute_delete (ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
				     &address, connected->ifp->ifindex, ifp->table);
#else
          rip_redistribute_delete (ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
                                   &address, connected->ifp->ifindex);
#endif
       ...
       "RIP version 2\n")
{
  ...
  ri = ifp->info;
	...
	if (atoi (argv[0]) == 1)
	{
		...
		ri->sc_rip_type = SC_RIP_1;
		return CMD_SUCCESS;
	}
	if (atoi (argv[0]) == 2)
	{
		ri->ri_send = RI_RIP_VERSION_2;
		if(strcmp(argv[0],"2M") == 0)
			ri->sc_rip_type = SC_RIP_2M;
		else if(strcmp(argv[0],"2B") == 0)
			ri->sc_rip_type = SC_RIP_2B;
		return CMD_SUCCESS;
	}
  return CMD_WARNING;
}
...

#ifdef __SC_BUILD__
DEFUN (rip_interface_multicast,
       rip_interface_multicast_cmd,
       "multicast",
       "config rip-2m\n"
       "Interface name\n")
{
  ...
  ri = ifp->info;
  ri->multicast = 1;
  zlog_debug("set %s to support rip-2m\n", ifp->name);
  return CMD_SUCCESS;
}
#endif /* __SC_BUILD__ */
...
#ifdef __SC_BUILD__
  install_element (INTERFACE_NODE, &config_interface_table_cmd);
  install_element (INTERFACE_NODE, &config_interface_main_cmd);
  install_element (INTERFACE_NODE, &config_interface_shared_cmd);
  install_element (INTERFACE_NODE, &rip_interface_multicast_cmd);
#endif
}
