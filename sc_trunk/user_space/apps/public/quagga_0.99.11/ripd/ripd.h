...
//  struct route_table *table[RT_TABLE_MAX];
  struct list *table_list;
  struct route_table *main;
#else
  /* RIP routing information base. */
...
struct rip_interface
{
  ...
  /* RIP version control. */
  int ri_send;
  //sercomm add to recognize 2B or 2M
  u_char sc_rip_type;//0:broadcast, 1:multicast
  int ri_receive;
...
#ifdef __SC_BUILD__
extern void rip_out_only_interface_clean ();
#endif
...
extern void rip_redistribute_add (int, int, struct prefix_ipv4 *, unsigned int, 
			   struct in_addr *, int, unsigned int, unsigned char);
extern void rip_redistribute_delete (int, int, struct prefix_ipv4 *, unsigned int, int);
#else
extern void rip_redistribute_add (int, int, struct prefix_ipv4 *, unsigned int, 
...
#ifdef __SC_BUILD__
extern void rip_zebra_ipv4_add (struct prefix_ipv4 *, struct in_addr *, u_int32_t, u_char, u_int32_t table);
#else
extern void rip_zebra_ipv4_add (struct prefix_ipv4 *, struct in_addr *, u_int32_t, u_char);
#endif
#ifdef __SC_BUILD__
extern void rip_zebra_ipv4_delete (struct prefix_ipv4 *, struct in_addr *, u_int32_t, u_int32_t table);
#else
...
struct rip_table_wraper {
  int id;
  struct route_table *table;
};

#define RIP_TABLE(__node) (((struct rip_table_wraper*)listgetdata(__node))->table)
#define RIP_TABLE_ID(__node) (((struct rip_table_wraper*)listgetdata(__node))->id)

extern struct route_table *rip_table(u_int32_t id);
