...
#ifdef __SC_BUILD__
      if (add_cmd)
        static_add_ipv4 (&p, NULL, NULL, ZEBRA_FLAG_BLACKHOLE, distance, zebrad.rtm_table_default);
      else
        static_delete_ipv4 (&p, NULL, NULL, distance, zebrad.rtm_table_default);
#else
      if (add_cmd)
...
#ifdef __SC_BUILD__
  if (add_cmd)
    static_add_ipv4 (&p, ifname ? NULL : &gate, ifname, flag, distance, zebrad.rtm_table_default);
  else
    static_delete_ipv4 (&p, ifname ? NULL : &gate, ifname, distance, zebrad.rtm_table_default);
#else
  if (add_cmd)
  ...
  for (node = listhead (vrf_list); node; nextnode (node))
    {
	  ...
	  if (table)
          {
                if(route_top (table))
		      vty_out (vty, "%% Table: %d%s", VRF_TABLE_ID(node), VTY_NEWLINE);
...
#ifdef __SC_BUILD__
  table = vrf_table (AFI_IP, SAFI_UNICAST, zebrad.rtm_table_default);
#else
  table = vrf_table (AFI_IP, SAFI_UNICAST, 0);
#endif
  if (! table)
...
#ifdef __SC_BUILD__
    vrf = vrf_lookup (zebrad.rtm_table_default);
#else
    vrf = vrf_lookup (0);
#endif
  if (! vrf)
...
#ifdef __SC_BUILD__
  /* Lookup table.  */
  stable = vrf_static_table (AFI_IP, SAFI_UNICAST, zebrad.rtm_table_default);
#else
...
#ifdef __SC_BUILD__
  if (add_cmd)
    static_add_ipv6 (&p, type, gate, ifname, distance, zebrad.rtm_table_default);
  else
    static_delete_ipv6 (&p, type, gate, ifname, distance, zebrad.rtm_table_default);
#else
  if (add_cmd)
...
#ifdef __SC_BUILD__
  table = vrf_table (AFI_IP6, SAFI_UNICAST, zebrad.rtm_table_default);
#else
...
static_config_ipv6 (struct vty *vty)
{
  ...
  /* Lookup table.  */
#ifdef __SC_BUILD__
  stable = vrf_table (AFI_IP6, SAFI_UNICAST, zebrad.rtm_table_default);
#else
  stable = vrf_table (AFI_IP6, SAFI_UNICAST, 0);
#endif
  if (! stable)
    return -1;
