...
#ifdef __SC_BUILD__
/* List for routing table. support more than 256 entry. */
...
vrf_lookup (u_int32_t id)
{
  ...
  for (node = listhead (vrf_list); node; nextnode (node))
    {
      vrf = listgetdata(node);
      if (vrf->id == id)
	return vrf;
    }
  
  /* add this new table. */
  sprintf(name, "IP-Routing-Table%d", id);
  vrf = vrf_alloc(name);
  vrf->id = id;
  zlog_info("Creating %s\n", name);
  listnode_add(vrf_list, vrf);
...
vrf_init ()
{
  ...
  listnode_add(vrf_list, default_table);
}
		     ...
		     struct route_node *top)
{
  ...
  table = vrf_table (AFI_IP, SAFI_UNICAST, RT_TABLE_MAIN);
#else
  ...
  {
      char buf1[BUFSIZ];
      zlog_debug("route %s/%d prefix in %s, rib->table = %d, rib->type = %d\n",
              inet_ntop (AF_INET, &p.prefix, buf1, BUFSIZ),
              p.prefixlen, __FUNCTION__, rib->table, rib->type);
  }
#endif
...
#ifdef __SC_BUILD__
  /* Lookup table.  */
  //table = vrf_table (AFI_IP6, SAFI_UNICAST, rib->table);
  table = vrf_table (AFI_IP6, SAFI_UNICAST, RT_TABLE_MAIN);
#else
...
rib_process (struct route_node *rn)
{
  ...
  for (rib = rn->info; rib; rib = next)
    {
       ...
       */
      next = rib->next;
      zlog_debug ("%s: %s/%d: rn %p, select %p fib %p del %p rib %p", __func__, buf, rn->p.prefixlen, 
            rn, select, fib, del, rib);
  ...
  if (IS_ZEBRA_DEBUG_RIB)
      zlog_debug ("%s: %s/%d: rn %p, select %p fib %p del %p", __func__, buf, rn->p.prefixlen, 
              rn, select, fib, del);
...
void rib_dump (const char * func, const struct prefix_ipv4 * p, const struct rib * rib)
{
  ...
  zlog_debug
  (
    "%s: refcnt == %lu, uptime == %ld, type == %u, table == %d",
    func,
    rib->refcnt,
    rib->uptime,
    rib->type,
...
#ifdef __SC_BUILD__
  /* Lookup table.  */
  table = vrf_table (AFI_IP6, SAFI_UNICAST, si->table);
#else
  /* Lookup table.  */
  table = vrf_table (AFI_IP6, SAFI_UNICAST, 0);
#endif
...
rib_update (void)
{
  ...
  for (node = listhead (vrf_list); node; nextnode (node))
    {
	    table = VRF_TABLE(node, AFI_IP, SAFI_UNICAST);
	    if (table)
	      for (rn = route_top (table); rn; rn = route_next (rn))
	        rib_process (rn);
	    table = VRF_TABLE(node, AFI_IP6, SAFI_UNICAST);
	    if (table)
	      for (rn = route_top (table); rn; rn = route_next (rn))
	        rib_process (rn);
    }
...
rib_weed_table (struct route_table *table)
{
  ...
  /*Martin@sdc.sercomm.com: we could filter other unused table later*/
  if (table)
    for (rn = route_top (table); rn; rn = route_next (rn))
      for (rib = rn->info; rib; rib = next)
	{
...
rib_weed_tables (void)
{
  ...
  for (node = listhead (vrf_list); node; nextnode (node))
    {
      rib_weed_table (VRF_TABLE(node, AFI_IP, SAFI_UNICAST));
      rib_weed_table (VRF_TABLE(node, AFI_IP6, SAFI_UNICAST));
    }
#else
...
rib_sweep_route (void)
{
  ...
  for (node = listhead (vrf_list); node; nextnode (node))
    {
      rib_sweep_table (VRF_TABLE(node, AFI_IP, SAFI_UNICAST));
      rib_sweep_table (VRF_TABLE(node, AFI_IP6, SAFI_UNICAST));
    }
#else    
...
rib_close (void)
{
  ...
  for (node = listhead (vrf_list); node; nextnode (node))
    {
      rib_close_table (VRF_TABLE(node, AFI_IP, SAFI_UNICAST));
      rib_close_table (VRF_TABLE(node, AFI_IP6, SAFI_UNICAST));
    }
#else
