...
#ifdef __SC_BUILD__
  /* Table */
  SET_FLAG (zapi_flags, ZAPI_MESSAGE_TABLE);
  stream_putl (s, rib->table);
#endif
  /* 
...
zread_ipv6_add (struct zserv *client, u_short length)
{
  ...
  if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
    rib_add_ipv6 (api.type, api.flags, &p, NULL, ifindex, api.table);
  else
    rib_add_ipv6 (api.type, api.flags, &p, &nexthop, ifindex, api.table);
#else    
  if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
...
#ifdef __SC_BUILD__    
  if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
    rib_delete_ipv6 (api.type, api.flags, &p, NULL, ifindex, api.table);
  else
    rib_delete_ipv6 (api.type, api.flags, &p, &nexthop, ifindex, api.table);
#else
  if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
