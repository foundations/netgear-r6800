...
#ifdef _MEM_TRACE_
		if (av_get_mt_size() > av_get_mt_thresh(0)) {
			av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info trying %d: use too much memory(%d), not go further!\n", count, av_get_mt_size());
			ret = count;
			break;
		}
#endif
