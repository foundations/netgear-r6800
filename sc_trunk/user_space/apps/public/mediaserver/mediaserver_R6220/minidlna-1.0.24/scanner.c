...
extern int av_get_mt_size();
extern void av_flush_mt();

static void segv_handler(int sig)
{
	DPRINTF_SC(E_DEBUG, L_SCANNER, "recv SIGSEGV\n");
	if (jmp_flag == (int)getpid())
		siglongjmp(sigjmp, 1);
	else
		exit(-1);
}
...
#ifdef _MEM_TRACE_
static int
__insert_file(char * name, const char * path, const char * parentID, int object)
#else
int insert_file(char * name, const char * path, const char * parentID, int object)
#endif
{
	char class[32];
	char objectID[64];
	...
	else if( is_video(name) )
	{
		...
		DPRINTF(E_INFO, L_SCANNER, "File %s is video by Name, Trying to get detailID.\n", path);
	...
	{
		DPRINTF(E_WARN, L_SCANNER, "File %s does not got detailID.\n", path);
...
int insert_file(char * name, const char * path, const char * parentID, int object)
{
	...
	DPRINTF_SC(E_DEBUG, L_SCANNER, "parsing file <%s><%s> !\n", name, path);
	
	if (sigsetjmp(sigjmp, 1) == 0) {
		jmp_flag = (int)getpid();
		ret = __insert_file(name, path, parentID, object);
		jmp_flag = 0;
		goto _out;
	} else {
	    jmp_flag = 0;
	    /* try to release some resource. but file/socket opened can't be released any more :( */
	    if (av_get_mt_size() > 0) {
	        DPRINTF_SC(E_DEBUG, L_SCANNER, "flush %d traced memory !\n", av_get_mt_size());
	        av_flush_mt();
	    }
		DPRINTF_SC(E_DEBUG, L_SCANNER, "insert_file SIGSEGV failure recover!\n");
		ret = -1;
	}
_out:
