...
#define FF_PROFILE_H264_CONSTRAINED_BASELINE (FF_PROFILE_H264_BASELINE|FF_PROFILE_H264_CONSTRAINED)
#endif
		...
		switch( vc->codec_id )
		{
			...
			case CODEC_ID_H264:
				...
				if( strcmp(ctx->iformat->name, "mpegts") == 0 )
				{
					...
					/* Do sanity check here, who knows how crazy some "media files" can be!!! peter, 2014.3.27 */
					if (!ctx->streams[video_stream]->r_frame_rate.den || !vc->time_base.den 
							|| !ctx->streams[video_stream]->r_frame_rate.num) {
						DPRINTF(E_ERROR, L_METADATA, "%s:%d Skip invalid/faking media file with r_frame_rate.den=%d, time_base.den=%d, r_frame_rate.num=%d!!!\n", __func__, __LINE__, 
								ctx->streams[video_stream]->r_frame_rate.den, vc->time_base.den, ctx->streams[video_stream]->r_frame_rate.num);
						break;
					}
