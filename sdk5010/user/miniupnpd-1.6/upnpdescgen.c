/* $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/miniupnpd-1.6/upnpdescgen.c#1 $ */
/* MiniUPnP project
...
#define WSCSERVICE_MAGICALVALUE (248)
#endif
...
static const char * const upnptypes[] =
{
	...
	"ui2",
	"ui4",
#ifdef ENABLE_WSC_SERVICE //Ralink :  be care that original code 4 => bin.base64, but portig code 4 => ui1
	"ui1",
#endif
	"bin.base64"
};
...
#if 0 //Ralink : seems useless!
static const char * const wps_upnpallowedvalues[] =
{
	0,		/* 0 */
	"",		/* 1 */
	0,
...
//Ralink :  be care origianl ver 1.2 poritng , 4 => ui1, 5 => bin.base64
#ifdef ENABLE_WSC_SERVICE
	{"A_ARG_TYPE_Base64", 5},
#else
	{"A_ARG_TYPE_Base64", 4},
...
static const struct stateVar WSCVars[] =
{
	...
	{"WLANEvent", 5|0x80, 0, 0, WSCSERVICE_MAGICALVALUE},    /* Conditional */ /*Ralink :  change magic value from 252 to WSCSERVICE_MAGICALVALUE*/
...
void get_model_name(void)
{
    ...
    if (*dev_name != '\0') {
        strcpy(model_name, dev_name);
    } else {
        strcpy(model_name, BOARD_ID);
    }
}
...
void get_model_description(void)
{
    ...
    if (*dev_name != '\0') {
        sprintf(model_description, "%s Gateway",dev_name);
    } else {
        sprintf(model_description, "%s Gateway",BOARD_ID);
    }
}

void get_wlan_friendly_name(void)
{
    char *dev_name = GetModelName();

    if (*dev_name != '\0') {
        sprintf(wlan_friendly_name, "%s (Wireless AP)",dev_name);
    } else {
        sprintf(wlan_friendly_name, "%s (Wireless AP)",BOARD_ID);
    }
}

void get_wlan_model_name(void)
{
    char *dev_name = GetModelName();

    if (*dev_name != '\0') {
        sprintf(wlan_model_name, "NETGEAR %s",dev_name);
    } else {
        sprintf(wlan_model_name, "NETGEAR %s",BOARD_ID);
    }
}

void get_wlan_model_num(void)
{
    char *dev_name = GetModelName();

    if (*dev_name != '\0') {
        strcpy(wlan_model_num, dev_name);
    } else {
        strcpy(wlan_model_num, BOARD_ID);
    }
}
#endif
