...
#if 1
                        /* SC_BUILD
                         * To fix mantis bug:47449. When l2tp set to dial on demand mode,
                         *                                              wan can't be triggerd after idle time is up.
                         * Root cause: l2tpd kill pppd
                         * Solution:  When idle time is up, restart wan to re-init all setup,
                         *                         when for nexttrigger info.
                         *                         (Only can do this for dial on demand mode.)
                         */
                        if(access("/tmp/l2tp_dod_mode", F_OK) == 0)
                        {
                        	system("/usr/sbin/echo call rc wan restart in openl2tp since this is dod > /dev/console");
                        	
                        	if (access("/tmp/l2tp_debug", F_OK) == 0)
                        	{
                                system("/usr/sbin/rc wan restart &");
                              } else
                              {
                              	system("/usr/sbin/rc wan l2tp 1 &");
                              }
                        } else
                        {
                        	system("/usr/sbin/echo call kill pppd in openl2tp > /dev/console");
                                kill(pid, SIGTERM);
                        }
#else			
			kill(pid, SIGTERM);
...
out:
	/* Added by Steven Liu
	 * When ppp link is up, we have to cancel setup timer immediately.
	 * If we don't, the ppp link will be terminated in 2 mins.
	 */
	if (ppp->setup_timer != NULL) {
