 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/ppp-2.4.2/pppd/plugins/pppopptp/pptp_callmgr.c#1 $
 */
#include <signal.h>
#include <sys/time.h>
#include <sys/types.h>
...
//				if(lci->pid[0] > 1) {SC_CFPRINTF("<< A pid[0] = %d >>\n", lci->pid[0]);kill(lci->pid[0], SIGTERM);}
//				if(lci->pid[1] > 1) {SC_CFPRINTF("<< B pid[1] = %d >>\n", lci->pid[1]);kill(lci->pid[1], SIGTERM);}
				kill(getpid(), SIGTERM);
            }
            break;
        default:
...
//					if(lci->pid[0] > 1) {SC_CFPRINTF("<< C pid[0] = %d >>\n", lci->pid[0]);kill(lci->pid[0], SIGTERM);}
//					if(lci->pid[1] > 1) {SC_CFPRINTF("<< D pid[1] = %d >>\n", lci->pid[1]);kill(lci->pid[1], SIGTERM);}
					kill(getpid(), SIGTERM);
                    free(lci);
...
shutdown:
	...
	signal(SIGTERM, callmgr_do_nothing);
        ...
        for (i = 0; i < vector_size(call_list); i++) {
            ...
            pptp_call_close(conn, call);
//			if(lci->pid[0] > 1) {SC_CFPRINTF("<< E pid[0] = %d >>\n", lci->pid[0]);kill(lci->pid[0], SIGTERM);}
//			if(lci->pid[1] > 1) {SC_CFPRINTF("<< F pid[1] = %d >>\n", lci->pid[1]);kill(lci->pid[1], SIGTERM);}
			kill(getpid(), SIGTERM);
        }
