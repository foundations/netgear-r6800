...
"$Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/ppp-2.4.2/pppd/plugins/rp-pppoe/plugin.c#1 $";
...
/*
 * Although pppd can scan PPPoE options when pppd initialize , it doesn't save any configuration. 
 * Add a interface function to set PPPoE options.                                                 Add by Bollen
 */
...
PPPOEInitDevice(void)
{
    ...
    acName = set_PPPoE_option("rp_pppoe_ac");
    if (acName) {
	SET_STRING(conn->acName, acName);
    }
    pppd_pppoe_service = set_PPPoE_option("rp_pppoe_service");
...
PPPOEConnectDevice(void)
{
    ...
    existingSession = set_PPPoE_option("rp_pppoe_sess");
    if (existingSession) {
