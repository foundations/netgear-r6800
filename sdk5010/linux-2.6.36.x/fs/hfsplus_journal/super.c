...
struct inode *hfsplus_journal_iget(struct super_block *sb, unsigned long ino)
{
    struct inode *inode;
...
static struct super_operations hfsplus_sops = {
	...
	.destroy_inode	= hfsplus_destroy_inode,
        /* FIXME: read_inode is obsoluted, will modify this */
...
static int hfsplus_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!HFSPLUS_SB(sb).cat_tree) {
        ...
        inode = hfsplus_journal_iget(sb, HFSPLUS_ALLOC_CNID);
	if (IS_ERR(inode)) {
            err = PTR_ERR(inode);
            if (!silent)
	...
	root = hfsplus_journal_iget(sb, HFSPLUS_ROOT_CNID);
        if (IS_ERR(root))
        {
	...
	if (!hfs_brec_read(&hfsplus_handle, &fd, &entry, sizeof(entry))) { 
		...
		HFSPLUS_SB(sb).hidden_dir = hfsplus_journal_iget(sb, be32_to_cpu(entry.folder.id));
		if (IS_ERR(HFSPLUS_SB(sb).hidden_dir)) {
...
{
    return get_sb_bdev(fs_type, flags, dev_name, data, hfsplus_fill_super, mnt);
}
