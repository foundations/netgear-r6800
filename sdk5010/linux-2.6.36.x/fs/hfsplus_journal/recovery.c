...
void hfsplus_jbd_brelse_array(struct buffer_head *b[], int n)
{
	while (--n >= 0)
		brelse (b[n]);
}
...
int hfsplus_jbd_recover(hfsplus_jbd_t *journal)
{
	...
	hfsplus_jbd_debug(0, "JBD: recovery, exit status %d, "
		  "recovered transactions %u to %u\n",
		  err, info.start_transaction, info.end_transaction);
	hfsplus_jbd_debug(0, "JBD: Replayed %d and revoked %d/%d blocks\n", 
			...
			struct recovery_info *info, enum passtype pass)
{
	...
	while (1) {
		...
		hfsplus_jbd_debug(3, "JBD: checking block %ld\n", next_log_block);
