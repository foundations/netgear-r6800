...
static void write_one_revoke_record(hfsplus_jbd_t *, hfsplus_transaction_t *,
				    ...
				    struct hfsplus_jbd_revoke_record_s *);
static void flush_descriptor(hfsplus_jbd_t *, struct hfsplus_jbd_head *, int);
#endif

/* Utility functions to maintain the revoke table */

/* Borrowed from buffer.c: this is a tried and tested block hash function */
static inline int hash(hfsplus_jbd_t *journal, unsigned long block)
{
...
static int insert_revoke_hash(hfsplus_jbd_t *journal, unsigned long blocknr,
...
int hfsplus_jbd_init_revoke(hfsplus_jbd_t *journal, int hash_size)
{
	...
	journal->j_revoke_table[0] = kmem_cache_alloc(hfsplus_revoke_table_cache, GFP_KERNEL);
	if (!journal->j_revoke_table[0])
		return -ENOMEM;
	journal->j_revoke = journal->j_revoke_table[0];

	/* Check that the hash_size is a power of two */
	HFSPLUS_J_ASSERT ((hash_size & (hash_size-1)) == 0);
	...
	if (!journal->j_revoke->hash_table) {
		kmem_cache_free(hfsplus_revoke_table_cache, journal->j_revoke_table[0]);
	...
	journal->j_revoke_table[1] = kmem_cache_alloc(hfsplus_revoke_table_cache, GFP_KERNEL);
	...
	if (!journal->j_revoke->hash_table) {
		kfree(journal->j_revoke_table[0]->hash_table);
		kmem_cache_free(hfsplus_revoke_table_cache, journal->j_revoke_table[0]);
		kmem_cache_free(hfsplus_revoke_table_cache, journal->j_revoke_table[1]);
		journal->j_revoke = NULL;
		return -ENOMEM;
	}
...
int hfsplus_jbd_revoke(hfsplus_jbd_handle_t *handle, unsigned long blocknr, 
		   struct buffer_head *bh_in)
{
	...
	hfsplus_jbd_debug(2, "insert revoke for block %lu, bh_in=%p\n", blocknr, bh_in);
