...
static void hfsplus_jbd_start_thread(hfsplus_jbd_t *journal)
{
...
printk("@@@@@@@ Oops! .. Need to check it function: %s, Line: %d\n", __FUNCTION__, __LINE__);
	...
	if (need_copy_out && !done_copy_out) {
		...
		tmp = hfsplus_jbd_rep_kmalloc(bh_in->b_size, GFP_NOFS);
		hfsplus_jbd_lock_bh_state(bh_in);
		if (jh_in->b_frozen_data) {
			kfree(tmp);
			goto repeat;
	...
	*jh_out = new_jh;
	...
	hfsplus_jbd_file_buffer(jh_in, transaction, HFSPLUS_BJ_Shadow);
...
int hfsplus_jbd_next_log_block(hfsplus_jbd_t *journal, unsigned long *retp)
{
 ...
 * ready.
 */
int hfsplus_jbd_bmap(hfsplus_jbd_t *journal, unsigned long blocknr, 
		 unsigned long *retp)
{
	int err = 0;
	unsigned long ret;
...
static hfsplus_jbd_t * hfsplus_jbd_init_common (void)
{
	...
	journal = hfsplus_jbd_kmalloc(sizeof(*journal), GFP_KERNEL);
	if (!journal)
		goto fail;
...
static int hfsplus_jbd_reset(hfsplus_jbd_t *journal)
{
	...
	hfsplus_jbd_start_thread(journal);
	return 0;
}
 ...
 * journaled object.
 */
void hfsplus_jbd_destroy(hfsplus_jbd_t *journal)
{
...
static int hfsplus_jbd_init_journal_head_cache(void)
{
	...
	HFSPLUS_J_ASSERT(hfsplus_jbd_head_cache == 0);
