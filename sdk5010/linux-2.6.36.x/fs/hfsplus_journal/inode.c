...
//	ret = block_write_begin_for_hfs(file, mapping, pos, len, flags, pagep, fsdata, hfsplus_get_block);
	ret = block_write_begin(mapping, pos, len, flags, pagep, hfsplus_get_block);
	if (ret)
		hfsplus_journal_stop(&hfsplus_handle);
				...
				struct page *page, void *fsdata)
{
	 ...
	 */
        /*FIXME: should use pos or calculate pos?*/
	/* pos = ((loff_t)page->index << PAGE_CACHE_SHIFT) + to; */

	ret = hfsplus_walk_page_buffers(hfsplus_handle, page_buffers(page), pos, pos+len, &partial, hfsplus_commit_write_fn);
	if (!partial)
...
struct address_space_operations hfsplus_journalled_btree_aops = {
...
struct address_space_operations hfsplus_journalled_aops = {
	.readpage	= hfsplus_readpage,
	.writepage	= hfsplus_journalled_writepage,
	.sync_page	= block_sync_page,
	.write_begin	= hfsplus_journalled_write_begin,
