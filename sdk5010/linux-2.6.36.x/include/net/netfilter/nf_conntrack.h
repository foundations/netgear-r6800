...
struct nf_conn {
	...
	uint8_t from_wan;
	uint8_t need_passthru;		/* for tcp vanilla port scan */
	uint8_t need_recheck;		/* As some existed conntrack need block, some needn't block */
	uint8_t clamp_confirmed;  	/* in case some ct may be confirmed again and again */
	struct list_head tcp_syn_bysrc_ip;	/* used by tcp syn flood */
	struct bysrc_ip_s *tcp_syn_head;	/* point to syn private list head */
	struct list_head clamp_byremote_ip;	/* used by clamp total session from one host */
	struct byremote_ip_s *clamp_head;	/* point to clamp private list head */
	uint8_t local_service;  	/* in case some ct may be confirmed again and again */
#endif
#endif
