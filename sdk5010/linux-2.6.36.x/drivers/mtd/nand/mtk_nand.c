...
struct sc_fl_parts_tbl
{
   	int index; /*the hard code logic map index*/
    int real_offs; /*offset in nand*/
    int real_size; /*size in nand number*/
};

static struct sc_fl_parts_tbl sc_part_table[SC_PARTITION_NUM];
#endif
...
static int is_skip_bad_block(struct mtd_info *mtd, int page)
{
	...
	count ++;
#ifdef CONFIG_SCM_SUPPORT
	if ((page << chip->page_shift) >= sc_part_table[SC_PARTITION_NUM-1].real_offs) 
	{
		return 0;
	}	
#else	
...
struct sc_fl_parts_tbl * temp=NULL;
char *buffer=NULL;
char *sc_buf=NULL;
int j=0;
int ii=0;

sc_buf=(u8*)kmalloc(sizeof(SC_FL_PARTS_TBL_MAGIC),GFP_KERNEL);

for(j=0;j<10;j++)
{
    memset(sc_buf,0,sizeof(SC_FL_PARTS_TBL_MAGIC));
    printk("Go to read Magic at %x\n", SC_MTD_PART_MAP_OFFSET+j*BLOCK_size);
	sc_ranand_read(sc_buf,SC_MTD_PART_MAP_OFFSET+j*BLOCK_size,sizeof(SC_FL_PARTS_TBL_MAGIC));
	if(memcmp(sc_buf, SC_FL_PARTS_TBL_MAGIC, strlen(SC_FL_PARTS_TBL_MAGIC))==0)
	{
    	printk("SC_DEBUG: NAND Partition Table Magic Fount at %x.\n",SC_MTD_PART_MAP_OFFSET+j*BLOCK_size);

		kfree(sc_buf);

	    buffer=(u8*)kmalloc(sizeof(struct sc_fl_parts_tbl),GFP_KERNEL);

    	for (ii=0;ii<SC_PARTITION_NUM;ii++)
		{
            memset(buffer,0,sizeof(struct sc_fl_parts_tbl));
			sc_ranand_read(buffer,SC_MTD_PART_MAP_OFFSET+j*BLOCK_size+PAGE_size+ii*sizeof(struct sc_fl_parts_tbl),sizeof(struct sc_fl_parts_tbl));
            temp = (struct sc_fl_parts_tbl*)(buffer);
            sc_part_table[ii].index=ii;
            sc_part_table[ii].real_offs=temp->real_offs;
            sc_part_table[ii].real_size=temp->real_size;
            printk("part %d,real_offset %08x, real_length %08x\n",ii,sc_part_table[ii].real_offs,sc_part_table[ii].real_size);
        }
        kfree(buffer);
		break;
    }
...
for (i=0;i<SC_PARTITION_NUM;i++)
{
   	g_pasStatic_Partition[i].size = sc_part_table[i].real_size;
   	g_pasStatic_Partition[i].offset = sc_part_table[i].real_offs;
	//printk("name=%s,size=%08x,offset=%08x\n",g_pasStatic_Partition[i].name,g_pasStatic_Partition[i].size,g_pasStatic_Partition[i].offset);
}
#endif    
/*********************************************************************************************/
#ifndef CONFIG_SCM_SUPPORT
...
#ifdef CONFIG_SCM_SUPPORT
/*
 * Noticed: this func do not skip bad block
 * The only way to check data is check the Magci word
 * make sure Set the Magic word at the first page of a Block
 * */
int sc_ranand_read(char *buf, unsigned int from, int datalen)
{
	...
	while (datalen) {
		...
		{			
#if 1
			printk("Read block fail! Just return!\n");
			return 0;
#else
			addr += mtd->erasesize;
			printk("jump to %x\n", (unsigned int)addr);
