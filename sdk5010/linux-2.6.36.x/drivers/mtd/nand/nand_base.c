...
#if (defined (CONFIG_MTD_ANY_RALINK) && defined (CONFIG_RALINK_MT7621))
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	if (mtd->ecc_stats.corrected - stats.corrected)
		printk("from %x, len=%x, corrected=%d %d\n", (unsigned int) from, (unsigned int) ops->len, mtd->ecc_stats.corrected, stats.corrected);
