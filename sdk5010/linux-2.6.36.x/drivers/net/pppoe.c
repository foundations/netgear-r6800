...
static unsigned char pppoe_server_addr[ETH_ALEN];
static unsigned long pppoe_sid;
/*
 * Send PADT if session id not match
 */
#define PADT_PACKET_LEN     60
static void pppoe_sendpadt(struct sk_buff *recv_skb, struct net_device *dev)
{
	struct sk_buff *skb;
	struct pppoe_hdr hdr;
    struct pppoe_hdr *ph;
	struct pppoe_hdr *recv_ph;
	struct pppoe_tag *tag;

    recv_ph = pppoe_hdr(recv_skb);
    ...
    skb = dev_alloc_skb(32 + PADT_PACKET_LEN + dev->hard_header_len);
    ...
    skb_reserve(skb, 32 + dev->hard_header_len);
    skb_reset_network_header(skb);
	dev_hard_header(skb, dev, ETH_P_PPP_DISC,
			 eth_hdr(recv_skb)->h_source, NULL, PADT_PACKET_LEN);


	ph = (struct pppoe_hdr *) skb_put(skb, PADT_PACKET_LEN);
	tag = &ph->tag[0];

    memset(ph, 0, PADT_PACKET_LEN);
	memcpy(ph, &hdr, sizeof(struct pppoe_hdr));
    /* Add Generic Error tag */
	tag->tag_type = PTT_GEN_ERR;
	tag->tag_len = __constant_htons(0);
	ph->length = __constant_htons(4);
		     ...
		     struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (po != NULL) {
         ...
         *                                              2010-05-20 11:36:37
         */
        if(memcmp(pppoe_server_addr, eth_hdr(skb)->h_source, ETH_ALEN) || ph->sid != pppoe_sid) {
            printk("Argon : <0> get PPPoE packet from %02X:%02X:%02X:%02X:%02X:%02X with invalid SID <0x%04X>, sending PADT \n",
...
#if 0
#ifdef CONFIG_SCM_SUPPORT
    /* Dirty solution: We need to record SID and Server MAC now. then pppoe_rcv() need to use */
    if (ph->code == PADS_CODE) {
/*
        printk("<0> get PADS packet from %02X:%02X:%02X:%02X:%02X:%02X with SID <0x%04X> \n",
            eth_hdr(skb)->h_source[0], eth_hdr(skb)->h_source[1],
            eth_hdr(skb)->h_source[2], eth_hdr(skb)->h_source[3],
            eth_hdr(skb)->h_source[4], eth_hdr(skb)->h_source[5], ntohs(ph->sid));
*/
        memcpy(pppoe_server_addr, eth_hdr(skb)->h_source, ETH_ALEN);
        pppoe_sid = ph->sid;
    }
#endif
#endif
