...
#warning "SC_SWITCH_USE_GMAC2 is defined!!! sc_tm_hook will take effect in RAETH..."
#define	KERNEL_LIMIT	/*for traffic meter exact limit*/ 
#ifdef KERNEL_LIMIT
#define WAN_IFNAME WAN_PHY_IFNAME
int wan_dev_index;
int (*sc_tm_hook_rx)(struct net_device *dev, int len) = NULL;
int (*sc_tm_hook_tx)(struct net_device *dev, int len) = NULL;
#endif
#else
#warning "SC_SWITCH_USE_GMAC2 is NOT defined!!! sc_tm_hook will NOT take effect in RAETH..."
#endif /* SC_SWITCH_USE_xxxx */
#endif
...
#ifdef KERNEL_LIMIT
 	if(sc_tm_hook_rx) {
    	if( !strcmp(rx_skb->dev->name, WAN_IFNAME))
		{
      		if(sc_tm_hook_rx(rx_skb->dev, rx_skb->len))
    		{
				sc_tm_rx_err = 1;
    		}
    	}
...
#ifdef CONFIG_SCM_SUPPORT
/* in some case, printk too much so system resouces are all taken by printk
 * we need to mark the printk but we still need to know rate of this printk.
 */
unsigned int sc_mii_write_counter = 0;
#endif		
			 ...
			 struct net_device * dev)
{
	...
	if(sc_tm_hook_tx)
	{
		if (!strcmp(pSkb->dev->name, WAN_IFNAME)) {
			if(sc_tm_hook_tx(pSkb->dev, pSkb->len))
			{
				kfree_skb(pSkb);
				pPesueoAd->stat.tx_dropped++;
				return 0;
			}
		}
	}
...
#ifdef CONFIG_SCM_SUPPORT
#ifdef KERNEL_LIMIT
EXPORT_SYMBOL(sc_tm_hook_rx);
EXPORT_SYMBOL(sc_tm_hook_tx);
EXPORT_SYMBOL(wan_dev_index);
#endif
#endif
