...
#include "usb.h"
  ...
  int (*sc_usbserial_filter_hook)(struct usb_device *dev) = NULL;
  EXPORT_SYMBOL(sc_usbserial_filter_hook);
  #endif
...
#ifdef CONFIG_SCM_SUPPORT
    if (strstr(driver->name, "NetUSB")
          && sc_usbserial_filter_hook && sc_usbserial_filter_hook(udev) == 1) {
          printk("ignore kcode driver <%s>! for dev <%s>\n", driver->name, dev_name(dev));
          return -ENODEV;
    }
#endif
