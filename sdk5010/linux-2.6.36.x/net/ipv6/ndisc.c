...
extern int sc_ipv6_trace_enable; 
...
#ifdef CONFIG_SCM_SUPPORT
extern int is_wandev(struct net_device *dev);
#endif
...
static void ndisc_router_discovery(struct sk_buff *skb)
{
	...
	if (in6_dev == NULL) {
        ...
        /* WAN intf needs to be set accept_ra and send rs */
#ifdef CONFIG_SCM_SUPPORT
	if (!accept_ra(in6_dev) && !(is_wandev(in6_dev->dev))) {
#else
	if ((in6_dev->cnf.forwarding && !(is_wandev(in6_dev->dev))) || 
	...
	if (in6_dev->cnf.accept_ra_pinfo && ndopts.nd_opts_pi) {
		...
		for (p = ndopts.nd_opts_pi;
		     ...
		     p = ndisc_next_option(p, ndopts.nd_opts_pi_end)) {
		     	SC_IPV6_TRACE("%s %d: CALL PREFIX RCV, xxx\n", __FUNCTION__, __LINE__);
