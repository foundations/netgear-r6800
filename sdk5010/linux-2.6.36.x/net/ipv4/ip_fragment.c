...
int sysctl_ipfrag_count_per_ip_max __read_mostly = 32;
    ...
    */
	...
	if(end + ihl > 65535)
	{
		if(qp->protocol == IPPROTO_TCP || qp->protocol == IPPROTO_UDP)
        	NETGEAR_SYSLOG("19 00[DoS attack: Jolt2 Attack] from source: %u.%u.%u.%u:%hu\n",
                            NIPQUAD(qp->saddr), qp->sport);
        else
        	NETGEAR_SYSLOG("19 00[DoS attack: Jolt2 Attack] from source: %u.%u.%u.%u\n",
                            NIPQUAD(qp->saddr));
...
#ifdef CONFIG_SCM_SUPPORT
#ifdef CONFIG_SPI_FIRWALL
	if( goto_teardrop && g_spi_fun_switch && (skb->dev)
		&& (skb->dev->ifindex == g_spi_wanindex)
		&& (qp->tuple.src.l3num == PF_INET)
		&& sc_check_and_block_hook)
	{
		int check_block;
		goto_teardrop = 0;
		check_block = sc_check_and_block_hook(&qp->tuple);
//			printk(KERN_EMERG "check_block = %d \n", check_block);
		if(check_block != DETECT_PASS)
    	{
			if(qp->protocol == IPPROTO_TCP || qp->protocol == IPPROTO_UDP)
    			NETGEAR_SYSLOG("19 00[DoS attack: Teardrop Attack] from source: %u.%u.%u.%u:%hu\n",
                        NIPQUAD(qp->saddr), qp->sport);
            else
    			NETGEAR_SYSLOG("19 00[DoS attack: Teardrop Attack] from source: %u.%u.%u.%u\n",
                        NIPQUAD(qp->saddr));
...
out_oversize:
	if (net_ratelimit())
	{
		...
		if(qp->protocol == IPPROTO_ICMP)
        	NETGEAR_SYSLOG("19 00[DoS attack: Ping of Death Attack] from source: %u.%u.%u.%u\n",
                                    NIPQUAD(qp->saddr));
#endif
#endif
...
#ifdef CONFIG_SCM_SUPPORT
#ifdef CONFIG_SPI_FIRWALL
	if(atomic_read(&ip_frag_total_count) >= sysctl_ipfrag_count_max)
		goto end;
#endif
#endif
