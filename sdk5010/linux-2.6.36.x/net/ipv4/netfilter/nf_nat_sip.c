...
static int map_addr(struct sk_buff *skb, unsigned int dataoff,
            ...
            /*For Call-ID header, there would be no port, and need not check that*/
            ,unsigned char check_port
#endif
		    )
{
	enum ip_conntrack_info ctinfo;
...
#ifdef CONFIG_SCM_SUPPORT
        (!check_port ||ct->tuplehash[dir].tuple.src.u.udp.port == port)
#else
...
#ifdef CONFIG_SCM_SUPPORT
        (!check_port || ct->tuplehash[dir].tuple.dst.u.udp.port == port)
#else	    
...
#else
    /*if there is no port in this header, do not add it*/
	{
		char tmp[sizeof("nnn.nnn.nnn.nnn:nnnnn")] = "";
		char *pt = tmp;
		unsigned char has_port = 0;;
		strncpy(tmp,*dptr+matchoff,matchlen);
		if(strchr(pt,':'))
		{
			has_port = 1;
		}
		if(!check_port || (has_port == 0 && newport == htons(SIP_PORT)))
		{/*do not add port in this header*/
			buflen = sprintf(buffer, "%pI4", &newaddr);
		}
		else
		{
			buflen = sprintf(buffer, "%pI4:%u", &newaddr, ntohs(newport));
			...
			enum sip_header_types type)
{
	...
	if (ct_sip_parse_header_uri(ct, *dptr, NULL, *datalen, type, NULL,
		...
		return 1;
	return map_addr(skb, dataoff, dptr, datalen, matchoff, matchlen,&addr, port
#ifdef CONFIG_SCM_SUPPORT
        ,type == SIP_HDR_CALLID?0:1
#endif
	);
}
...
#ifdef CONFIG_SCM_SUPPORT
	    || !map_sip_addr(skb, dataoff, dptr, datalen, SIP_HDR_CALLID)
#endif
	    )
		return NF_DROP;
...
#else
		{/* if there was no port, do not add it*/
		    char tmp[sizeof("nnn.nnn.nnn.nnn:nnnnn")] = "";
		    char *pt = tmp;
		    unsigned char has_port = 0;;
		    strncpy(tmp,*dptr+matchoff,matchlen);
		    ...
		    if(has_port == 0 && port == htons(SIP_PORT))
		    {/*do not add port in this header*/
				buflen = sprintf(buffer, "%pI4", &newip);
		    }
		    else
		    {
				buflen = sprintf(buffer, "%pI4:%u", &newip, ntohs(port));
			}
		}
#endif
		if (!mangle_packet(skb, dataoff, dptr, datalen,
