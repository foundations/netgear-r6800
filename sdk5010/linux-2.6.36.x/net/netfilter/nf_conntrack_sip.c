...
#ifdef CONFIG_SCM_SUPPORT
	[SIP_HDR_CALLID]			= SIP_HDR("Call-ID", "i", NULL, skp_epaddr_len),
#endif
};
...
#ifdef CONFIG_SCM_SUPPORT
static int process_via(struct sk_buff *skb, unsigned int dataoff, unsigned int protoff,
		       struct nf_conn *ct, enum ip_conntrack_info ctinfo,
		       char **dptr, int *dlen)
{
	...
	ret = ct_sip_parse_header_uri(ct, *dptr, NULL, *dlen,
	...
	if(ret > 0) 
	{
		...
		DEBUGP("nf_conntrack_sip: Via=%.*s\n", len, p);
    	memcpy(&addr.u3,&daddr,sizeof(daddr));
    	memcpy(&addr.u.udp.port,&port,sizeof(port));
		...
		if (!memcmp(&addr.u3, &ct->tuplehash[dir].tuple.src.u3,
		    ...
		    addr.u.udp.port != ct->tuplehash[dir].tuple.src.u.udp.port){
		    	DEBUGP("nf_conntrack_sip: different message port\n");
...
//					 &p, &len, &addr);
        	...
        	{
        	    DEBUGP("need do nf_nat_sip_expect\n");
        		ret = nf_nat_sip_expect(skb, dataoff, dptr, dlen, exp,
        					matchoff, matchlen);
        				 
        	} else {
        		if (nf_ct_expect_related(exp) != 0) {
        			DEBUGP("nf_conntrack_sip: nf_ct_expect_related() "
        				 "failed\n");
        		    ret = NF_DROP;
        		}
        		else
		...
		/* LAN->WAN. Change the LAN address to WAN address */ 
			...
			ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr,
					  dlen, &p, &len, &addr);
		    	DEBUGP("nf_conntrack_sip: LAN address in Via "
		 ...
		 * set by some 'smart' UAs. We need to change the port. */ 
			...
			ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr,
					  dlen, &p, &len, &addr);
		    	DEBUGP("nf_conntrack_sip: Auto-detected WAN address "
			       	 "in Via changed to %.*s\n", len, p);
			return ret;
		}
		/* WAN->LAN. Change the WAN address to LAN address */ 
			...
			ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr,
					  dlen, &p, &len, &addr);
		    	DEBUGP("nf_conntrack_sip: WAN address in Via "
...
#ifdef CONFIG_SCM_SUPPORT
static int process_bye_response(struct sk_buff *skb,
			       const char **dptr, unsigned int *datalen,
			       unsigned int cseq, unsigned int code)
{
	enum ip_conntrack_info ctinfo;
	...
	if (help->help.ct_sip_info.bye_cseq != cseq)
		return NF_ACCEPT;
#ifdef CONFIG_SCM_SUPPORT
			       ...
			       unsigned int cseq)
{
	...
	if (!list_empty(&ct->derived_connections)) {
		...
		list_for_each_entry_safe(child, tmp, &ct->derived_connections,
			derived_list) {
	    DEBUGP("clearing child %p\n",child);
		child->derived_timeout = 3;
		nf_ct_refresh(child, skb, 3*HZ);
	}
    }
	help->help.ct_sip_info.bye_cseq = cseq;
#endif
	flush_expectations(ct, true);
	return NF_ACCEPT;
}
...
static const struct sip_handler sip_handlers[] = {
	...
	SIP_HANDLER("BYE", process_bye_request, 
#ifdef CONFIG_SCM_SUPPORT
	process_bye_response
#else
    NULL
#endif
	),
...
#ifdef CONFIG_SCM_SUPPORT
	SIP_HANDLER("CANCEL", process_bye_request, NULL),
#endif
};
...
/*Port from WNR2000v3, for mantis 0019824: SIP ALG doesn't support IP fragment */
#ifdef CONFIG_SCM_SUPPORT
	/* support SIP message over IP fragments */
	if (!skb_linearize(skb))
		dptr = skb->data + dataoff;
	else {
		DEBUGP("skb_linearize return no free memory\n");
		return NF_ACCEPT;
	}
#else
	...
	ret = process_via(skb, dataoff, protoff, ct, ctinfo, &dptr, &datalen);
	if (ret != NF_ACCEPT)
		return ret;
				...
				unsigned long count, void *data)
{
	...
	DEBUGP("file: %08x, buffer: %s, count: %lu, data: %08x\n",
...
static int __init nf_conntrack_sip_init(void)
{
    ...
    res = create_proc_entry("sipalg_enable", 0, NULL);    
	if (res) {
		res->read_proc = read_proc;
		res->write_proc = write_proc;
	}
	else {
		printk("SIP Alg, create proc failed!\n");
	}
#endif
